...
2. That agent was named `Legacy-VIBEK-PM-sleeptime` (wrong name)
3. The agent's memory showed project "VIBEK" (wrong project)
4. This was a leftover from the agent reuse issue

## Root Cause

When we renamed all the mismatched agents earlier, we renamed them based on their memory blocks. However, BKMCP was mapped to a VIBEK sleeptime agent. When we renamed it to `Legacy-BKMCP-PM-sleeptime`, we corrected the name but:
...
```bash
Legacy-VIBEK-PM-sleeptime → Legacy-BKMCP-PM-sleeptime
```

### Step 2: Created Primary Agent
Created new primary agent:
- Name: `Legacy-BKMCP-PM`
...
- **Name**: `Legacy-BKMCP-PM-sleeptime`
...
1. **Primary Agent** (`Legacy-{PROJECT}-PM`)
   - Used for direct interaction and queries
   - Maintains current project state
   - Handles real-time updates

2. **Sleeptime Agent** (`Legacy-{PROJECT}-PM-sleeptime`)
   - Runs in background
...
**Result**: ✅ BKMCP agent is now searchable in Letta as "Legacy-BKMCP-PM"
