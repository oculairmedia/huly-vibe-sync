...
function paginate<T>(items: T[], cursor?: string, limit = DEFAULT_PAGE_LIMIT): { items: T[]; next_cursor: string | null; has_more: boolean; total_known: number } {
  ...
  let startIdx = 0;
  if (cursor) { const idx = items.findIndex((item: Any) => (item as Record<string, unknown>).id === cursor || (item as Record<string, unknown>).identifier === cursor); if (idx >= 0) startIdx = idx + 1; }
  ...
  return { items: slice, next_cursor: lastItem && slice.length === effectiveLimit ? encodeCursor(String(lastItem.id || lastItem.identifier)) : null, has_more: startIdx + effectiveLimit < items.length, total_known: items.length };
}

export function registerProjectRoutes(app: Any, deps: Any): void {
  const { db, config: _config, parseJsonBody, sendJson, sendError, logger, projectRegistry, doltHubProvisioner, beadsIssueService, beadsAdapter } = deps;

  app.registerRoute({
    match: ({ pathname, method }: Any) => pathname === '/api/projects' && method === 'GET',
    handle: async ({ res, url }: Any) => {
      ...
      try {
        ...
        const projects = rows.map((p: Any) => serializeProject(p)).filter(Boolean);
  ...
  app.registerRoute({
    match: ({ pathname, method }: Any) => pathname.startsWith('/api/projects/') && pathname.endsWith('/issues') && method === 'GET',
    handle: async ({ res, url, pathname }: Any) => {
      ...
      try {
        ...
        if (statusFilter) { const statuses = statusFilter.split(',').map((s: string) => s.trim().toLowerCase()); filtered = filtered.filter(i => statuses.includes(normalizeIssueStatus(i.status as string))); }
        if (priorityFilter) { const priorities = priorityFilter.split(',').map((p: string) => p.trim().toLowerCase()); filtered = filtered.filter(i => priorities.includes(((i.priority as string) || 'medium').toLowerCase())); }
        ...
        if (updatedSince) { const since = toDateMs(updatedSince); if (since) filtered = filtered.filter(i => { const t = toDateMs(i.updatedAt || i.modifiedAt || i.updated_at); return t !== null && t >= since; }); }

        if (sort === 'updated') filtered.sort((a, b) => { const at = toDateMs(a.updatedAt || a.modifiedAt || a.updated_at) || 0; const bt = toDateMs(b.updatedAt || b.modifiedAt || b.updated_at) || 0; return bt - at; });
        else if (sort === 'created') filtered.sort((a, b) => { const at = toDateMs(a.createdAt || a.created_at) || 0; const bt = toDateMs(b.createdAt || b.created_at) || 0; return bt - at; });
        ...
        const trackerStats = { total: allIssues.length, filtered: serialized.length, open: allIssues.filter(i => normalizeIssueStatus(i.status as string) === 'open').length, inProgress: allIssues.filter(i => normalizeIssueStatus(i.status as string) === 'in_progress').length, closed: allIssues.filter(i => normalizeIssueStatus(i.status as string) === 'closed').length, blocked: allIssues.filter(i => normalizeIssueStatus(i.status as string) === 'blocked').length };
        const dataFreshness = { status: 'fresh' as const, last_sync_at: toIsoTimestamp(project.last_sync_at) };
  ...
  app.registerRoute({
    match: ({ pathname, method }: Any) => pathname.startsWith('/api/issues/') && !pathname.endsWith('/issues') && method === 'GET',
    handle: async ({ res, pathname }: Any) => {
  ...
  app.registerRoute({
    match: ({ pathname, method }: Any) => pathname.startsWith('/api/projects/') && pathname.endsWith('/ready-work') && method === 'GET',
    handle: async ({ res, url, pathname }: Any) => {
  ...
  app.registerRoute({
    match: ({ pathname, method }: Any) => pathname.startsWith('/api/projects/') && pathname.split('/').length === 4 && method === 'GET',
    handle: async ({ res, pathname }: Any) => {
      ...
      try {
        ...
        if (subresource && subresource !== '') { sendError(res, 404, 'Unknown subresource'); return; }
  ...
  app.registerRoute({
    match: ({ pathname, method }: Any) => pathname === '/api/registry/projects' && method === 'POST',
    handle: async ({ req, res }: Any) => {
      try {
        const body = await parseJsonBody(req);
        const { filesystem_path, name: _name, git_url: _git_url } = body;
        ...
        const project = projectRegistry?.registerProject ? projectRegistry.registerProject(filesystem_path) : null;
  ...
  app.registerRoute({
    match: ({ pathname, method }: Any) => pathname.startsWith('/api/registry/projects/') && pathname.split('/').length === 5 && method === 'GET',
    handle: async ({ res, pathname }: Any) => {
  ...
  app.registerRoute({
    match: ({ pathname, method }: Any) => pathname.startsWith('/api/projects/') && pathname.endsWith('/beads-remote/provision') && method === 'POST',
    handle: async ({ req, res, pathname }: Any) => {
  ...
  const ISSUE_MUTATION_ROUTES = ['claim', 'unclaim', 'close', 'reopen'] as const;
  for (const action of ISSUE_MUTATION_ROUTES) {
    app.registerRoute({
      match: ({ pathname, method }: Any) => pathname.startsWith('/api/issues/') && pathname.endsWith(`/${action}`) && method === 'POST',
      handle: async ({ req, res, pathname }: Any) => {
        ...
        try {
          ...
          const body = await parseJsonBody(req);
          let result: Any;
          if (beadsIssueService?.[`${action}Issue` as keyof typeof beadsIssueService]) {
            result = await (beadsIssueService as Any)[`${action}Issue`](issueId, { ...body, idempotencyKey });
          } else if (beadsAdapter) {
            result = await (beadsAdapter as Any)._handleBeadsAdapterMutation(action as string, issueId, body, beadsAdapter, idempotencyKey);
  ...
  app.registerRoute({
    match: ({ pathname, method }: Any) => pathname.startsWith('/api/issues/') && pathname.endsWith('/status') && method === 'PATCH',
    handle: async ({ req, res, pathname }: Any) => {
      ...
      try {
        ...
        const result = await beadsIssueService.updateIssueStatus(issueId, body.status, { idempotencyKey, ...body });
        sendJson(res, 200, result);
  ...
  app.registerRoute({
    match: ({ pathname, method }: Any) => pathname.startsWith('/api/issues/') && pathname.endsWith('/notes') && method === 'POST',
    handle: async ({ req, res, pathname }: Any) => {
      ...
      try {
        ...
        const result = await beadsIssueService.addIssueNote(issueId, body.content, { idempotencyKey, ...body });
        sendJson(res, 200, result);
