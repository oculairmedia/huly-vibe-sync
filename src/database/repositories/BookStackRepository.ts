...
export class BookStackRepository {
  ...
  getBookStackLastExport(projectIdentifier: string): number | null {
    const row = this.db
      ...
      .get(projectIdentifier) as { bookstack_last_export_at?: number } | undefined;
      ...
      .prepare(
        `INSERT INTO bookstack_pages
         ...
         ON CONFLICT(bookstack_page_id) DO UPDATE SET
           ...
           content_hash = excluded.content_hash, bookstack_modified_at = excluded.bookstack_modified_at,
           local_modified_at = excluded.local_modified_at, last_export_at = excluded.last_export_at,
           last_import_at = excluded.last_import_at, sync_direction = excluded.sync_direction,
           bookstack_content_hash = excluded.bookstack_content_hash, sync_status = excluded.sync_status,
           bookstack_revision_count = excluded.bookstack_revision_count, updated_at = ?`,
      )
      .run(
        page.bookstack_page_id, page.bookstack_book_id, page.bookstack_chapter_id || null,
        page.project_identifier || null, page.slug, page.title, page.local_path || null,
        page.content_hash || null, page.bookstack_modified_at || null,
        page.local_modified_at || null, page.last_export_at || null,
        page.last_import_at || null, page.sync_direction || 'export',
        page.bookstack_content_hash || null, page.sync_status || 'synced',
        page.bookstack_revision_count || null, now, now,
      );
  }
  ...
  deleteProjectPages(projectIdentifier: string): void {
