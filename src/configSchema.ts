...
const doltHubSchema = z
  ...
  .refine((data) => !data.enabled || data.dryRun || Boolean(data.apiToken), {
    message: 'DOLTHUB_API_TOKEN must be set when DoltHub provisioning is enabled',
  });
