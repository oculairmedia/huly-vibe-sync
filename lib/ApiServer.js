...
export function createApiServer({
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      // Route: POST /api/projects/:identifier/ast-sync - Trigger initial AST sync for a project
      if (
        pathname.startsWith('/api/projects/') &&
        pathname.endsWith('/ast-sync') &&
        method === 'POST'
      ) {
      ...
      // Route: POST /api/beads/label - Add/remove label on beads issue
      if (pathname === '/api/beads/label' && method === 'POST') {
        try {
      ...
      if (pathname === '/api/letta-code/configure-project' && method === 'POST') {
        ...
        try {
          ...
          const result = await lettaCodeService.configureForProject(
            agentId,
            hulyProject,
            agentName
          );
...
`);
      ...
      // Route: POST /api/temporal/reconciliation/run - Run data reconciliation
      if (pathname === '/api/temporal/reconciliation/run' && method === 'POST') {
        if (!getTemporalClient) {
          sendError(res, 503, 'Temporal orchestration not configured');
          return;
        }
