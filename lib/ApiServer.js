 ...
 */
export function createApiServer({ config, healthStats, db, onSyncTrigger, onConfigUpdate, lettaCodeService = null, webhookHandler = null, getTemporalClient = null }) {
    ...
    try {
      ...
      if (pathname === '/api/sync/trigger' && method === 'POST') {
        try {
          ...
          // Resolve project identifier if it might be a folder name
          // This handles cases where folder names like "lettatoolsselector" are passed
          ...
          if (projectId && db?.resolveProjectIdentifier) {
            ...
            if (resolvedId) {
              logger.info({ original: projectId, resolved: resolvedId }, 'Resolved folder name to project ID');
              projectId = resolvedId;
            } else {
              logger.warn({ projectId }, 'Could not resolve project identifier - may be invalid folder name');
              // Continue anyway - let the sync handle validation
            }
          }
...
`);
      ...
      // ============================================
      // TEMPORAL SCHEDULE MANAGEMENT ROUTES
      // ============================================

      // Route: GET /api/temporal/schedule - Get schedule status
      if (pathname === '/api/temporal/schedule' && method === 'GET') {
      ...
      // Route: POST /api/temporal/schedule/start - Start scheduled sync
      if (pathname === '/api/temporal/schedule/start' && method === 'POST') {
        if (!getTemporalClient) {
        ...
        try {
          ...
          const body = await parseJsonBody(req);
          const intervalMinutes = body.intervalMinutes || Math.max(1, Math.round(config.sync.interval / 60000));
      ...
      // Route: POST /api/temporal/schedule/stop - Stop scheduled sync
      if (pathname === '/api/temporal/schedule/stop' && method === 'POST') {
        if (!getTemporalClient) {
          sendError(res, 503, 'Temporal orchestration not configured');
          return;
        }
      ...
      // Route: PATCH /api/temporal/schedule - Update schedule interval
      if (pathname === '/api/temporal/schedule' && method === 'PATCH') {
        if (!getTemporalClient) {
          sendError(res, 503, 'Temporal orchestration not configured');
          return;
        }
      ...
      // Route: GET /api/temporal/workflows - List sync workflows
      if (pathname === '/api/temporal/workflows' && method === 'GET') {
        if (!getTemporalClient) {
          sendJson(res, 200, {
            available: false,
            message: 'Temporal orchestration not configured',
        ...
        try {
          ...
          // Import listSyncWorkflows dynamically
          const { listSyncWorkflows } = await import('../temporal/dist/client.js');
          const limit = parseInt(url.searchParams.get('limit') || '20');
