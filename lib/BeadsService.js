...
export async function syncHulyIssueToBeads(
  ...
  const currentBeadsModifiedAt = beadsIssue?.updated_at ? new Date(beadsIssue.updated_at).getTime() : null;
...
export async function syncBeadsToGit(projectPath, options = {}) {
  ...
  try {
    ...
    try {
      ...
      if (errorMsg.includes('no changes added to commit') || errorMsg.includes('nothing added to commit')) {
    ...
    try {
      ...
      console.warn(`[Beads] Could not push ${projectIdentifier} to remote: ${errorMsg.split('\n')[0]}`);
      return false;
    }
  } catch (error) {
