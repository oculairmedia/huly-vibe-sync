...
export async function initializeBeads(projectPath, options = {}) {
  try {
    ...
    // Run bd init with --quiet and --skip-hooks flags
    // --quiet: suppress output
    // --skip-hooks: skip git hooks installation (not needed for sync)
    // --skip-merge-driver: skip merge driver setup (not needed for sync)
    execBeadsCommand('init --quiet --skip-hooks --skip-merge-driver', projectPath);
...
export async function syncBeadsToGit(projectPath, options = {}) {
  const { projectIdentifier = 'unknown' } = options;
  ...
  try {
    ...
    // Ensure .beads/ directory is tracked in git
    // Check if .beads/ files are already tracked
    try {
      ...
      if (!lsFiles) {
        // .beads/ not tracked yet - add core files that beads expects to track
        console.log(`[Beads] Adding .beads/ directory to git tracking for ${projectIdentifier}`);
        execSync('git add .beads/.gitignore .beads/config.yaml .beads/metadata.json', {
          cwd: projectPath,
          encoding: 'utf-8',
          stdio: 'pipe',
        });
        
        // Create initial commit for beads setup
        try {
          execSync('git commit -m "chore(beads): initialize beads tracking"', {
            cwd: projectPath,
            encoding: 'utf-8',
            stdio: 'pipe',
          });
          console.log(`[Beads] Created initial beads setup commit for ${projectIdentifier}`);
        } catch (commitError) {
          // Commit might fail if there are git hooks or nothing to commit
          // This is okay, we'll try to sync anyway
        }
      }
    } catch (gitError) {
      // Git commands might fail in non-git repos or permission issues
      // Log but continue - let bd sync handle it
      console.warn(`[Beads] Could not check git tracking status: ${gitError.message}`);
    }
    ...
    // Use --no-push flag to prevent automatic push (we'll push separately to handle errors better)
    try {
      execBeadsCommand(`sync -m "${commitMessage}" --no-push`, projectPath);
    } catch (syncError) {
      // bd sync might fail if there are no changes, that's okay
      if (syncError.message.includes('no changes') || 
          syncError.message.includes('nothing to commit')) {
    ...
    try {
      ...
      if (pushError.message.includes('up-to-date') || 
          pushError.message.includes('Everything up-to-date')) {
      ...
      console.warn(`[Beads] Could not push ${projectIdentifier} to remote: ${pushError.message}`);
      return false;
    }
  } catch (error) {
