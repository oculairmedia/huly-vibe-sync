  ...
  try {
    ...
    try {
      ...
      if (!lsFiles) {
        ...
        execSync('git add .beads/.gitignore .beads/README.md .beads/config.yaml .beads/metadata.json', {
          cwd: projectPath,
          stdio: 'pipe',
        });
        
        // Commit with --no-verify to skip hooks (since this is just infrastructure setup)
        try {
          execSync('git commit --no-verify -m "chore(beads): initialize beads issue tracker"', {
            cwd: projectPath,
            stdio: 'pipe',
          });
          console.log(`[Beads] Committed .beads/ setup to git`);
        } catch (commitError) {
          // Commit might fail if hooks are strict, that's okay
...
export async function syncBeadsToGit(projectPath, options = {}) {
  ...
  try {
    ...
    // bd sync handles: export to JSONL → git add → git commit → git pull → import JSONL → git push
