...
export async function syncBeadsIssueToHuly(
  ...
  const { updateHulyIssueStatus } = await import('./HulyService.js');
  const { mapBeadsStatusToHuly } = await import('./statusMapper.js');
  const { extractHulyIdentifier } = await import('./textParsers.js');
  
  // Skip if this issue was just updated in Phase 3
  if (phase3UpdatedIssues.has(beadsIssue.id)) {
    console.log(`[Skip Beads→Huly] Issue ${beadsIssue.id} was just updated in Phase 3`);
    return;
  }
  ...
  if (!dbIssue) {
    // Issue not synced from Huly, skip
    return;
  }
  ...
  if (!hulyIssue) {
    console.warn(`[Beads→Huly] Huly issue ${hulyIdentifier} not found`);
    return;
  }
