 ...
 * Much faster than calling fetchHulyIssues for each project individually
 *
 * @param {Object} hulyClient - Huly REST API client
 * @param {string[]} projectIdentifiers - Array of project identifiers
 * @param {Object} config - Configuration object
 * @param {Object} db - Database instance (optional, for cursor-based incremental sync)
 * @returns {Promise<Object>} Map of projectId -> { issues, syncMeta }
 */
export async function fetchHulyIssuesBulk(hulyClient, projectIdentifiers, config = {}, db = null) {
  const startTime = Date.now();

  // Check if client supports bulk endpoint
  ...
  // Find the oldest sync cursor across all projects for the bulk query
  ...
  try {
    ...
    // Update sync cursors for each project
