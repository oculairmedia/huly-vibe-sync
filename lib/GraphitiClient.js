...
export class GraphitiClient {
  ...
  /**
   * Get edge UUID from Graphiti server.
   */
  async getEdgeUuid(sourceName, targetName, relationType, groupId = this.groupId) {
    const url = new URL(`${this.baseUrl}/api/utils/edge-uuid`);
    url.searchParams.set('source_name', sourceName);
    url.searchParams.set('target_name', targetName);
    url.searchParams.set('relation_type', relationType);
    url.searchParams.set('group_id', groupId);
