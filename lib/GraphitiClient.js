...
// DNS namespace UUID for UUID5 generation
...
export class GraphitiClient {
  ...
  /**
   * Normalize an entity name for UUID generation
   * Matches Graphiti's server-side normalization algorithm
   *
   * @param {string} name - Entity name (e.g., 'File:src/main.py')
   * @returns {string} Normalized name (e.g., 'file_src_main_py')
   */
  normalizeName(name) {
    return name
      .toLowerCase()
   ...
   * Uses UUID5 with group-specific namespace for consistent identity
   *
   * Algorithm:
   * 1. Create group namespace: UUID5(DNS_NAMESPACE, "graphiti.entity.{groupId}")
   * 2. Normalize entity name
   * 3. Generate entity UUID: UUID5(groupNamespace, normalizedName)
   *
   * @param {string} name - Entity name (e.g., 'File:src/main.py')
   * @param {string} [groupId] - Override group ID (defaults to instance groupId)
   * @returns {string} Deterministic UUID
   */
  generateEntityUuid(name, groupId = this.groupId) {
    const groupNamespace = uuidv5(`graphiti.entity.${groupId}`, DNS_NAMESPACE);
    const normalizedName = this.normalizeName(name);
    return uuidv5(normalizedName, groupNamespace);
  }
   ...
   * @param {string} name - Edge name/type (e.g., 'DEFINES')
   * @param {string} [groupId] - Override group ID
   * @returns {string} Deterministic edge UUID
   */
  generateEdgeUuid(sourceUuid, targetUuid, name, groupId = this.groupId) {
    const normalizedName = name.trim().toUpperCase() || 'RELATES_TO';
    const edgeKey = `${sourceUuid}:${targetUuid}:${normalizedName}:${groupId}`;
    const groupNamespace = uuidv5(`graphiti.edge.${groupId}`, DNS_NAMESPACE);
    return uuidv5(edgeKey, groupNamespace);
  }

  /**
   * Verify UUID generation matches server (for testing/debugging)
   *
   * @param {string} name - Entity name
   * @returns {Promise<Object>} Server response with uuid and normalized_name
   */
  async verifyUuid(name) {
    const url = new URL(`${this.baseUrl}/api/utils/uuid`);
    url.searchParams.set('name', name);
    url.searchParams.set('group_id', this.groupId);
  ...
  async upsertEntity(entity) {
    const uuid = entity.uuid || this.generateEntityUuid(entity.name);
