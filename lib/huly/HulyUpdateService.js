...
export async function updateHulyIssueParent(
  hulyClient,
  issueIdentifier,
  parentIdentifier,
  config = {}
) {
  const targetParent = parentIdentifier || null;
  const displayParent = targetParent || 'top-level';

  if (config.sync?.dryRun) {
    console.log(
      `[Huly] [DRY RUN] Would move issue ${issueIdentifier} to parent: ${displayParent}`
    );
    return true;
  }
  ...
  const startTime = Date.now();
  try {
    if (typeof hulyClient.moveIssue === 'function') {
      await hulyClient.moveIssue(issueIdentifier, targetParent);
    } else if (typeof hulyClient.setParentIssue === 'function') {
      await hulyClient.setParentIssue(issueIdentifier, targetParent);
    } else if (typeof hulyClient.callTool === 'function') {
      await hulyClient.callTool('huly_issue_ops', {
        operation: 'update',
        issue_identifier: issueIdentifier,
        update: { field: 'parentIssue', value: targetParent },
      });
    } else {
      throw new Error('Unsupported client type');
    }

    recordApiLatency('huly', 'moveIssue', Date.now() - startTime);
    console.log(`[Huly] ✓ Moved issue ${issueIdentifier} to parent: ${displayParent}`);
    return true;
  } catch (error) {
    recordApiLatency('huly', 'moveIssue', Date.now() - startTime);
    console.error(`[Huly] Error moving issue ${issueIdentifier}:`, error.message);
    return false;
  }
}
