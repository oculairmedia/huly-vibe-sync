...
const HULY_MIN_WRITE_INTERVAL_MS = Number(process.env.HULY_MIN_WRITE_INTERVAL_MS || 100);
const HULY_WRITE_MAX_RETRIES = Number(process.env.HULY_WRITE_MAX_RETRIES || 4);
const HULY_WRITE_BASE_BACKOFF_MS = Number(process.env.HULY_WRITE_BASE_BACKOFF_MS || 500);

let nextHulyWriteAt = 0;
...
async function throttleWrite() {
  const now = Date.now();
  const waitMs = Math.max(0, nextHulyWriteAt - now);
  nextHulyWriteAt = Math.max(nextHulyWriteAt, now) + HULY_MIN_WRITE_INTERVAL_MS;
  if (waitMs > 0) {
    await sleep(waitMs);
  }
}
...
async function performWriteWithBackoff(url, options) {
  for (let attempt = 0; attempt < HULY_WRITE_MAX_RETRIES; attempt++) {
    await throttleWrite();
    const response = await fetchWithPool(url, options);

    if (
      (response.status === 429 || response.status === 503) &&
      attempt < HULY_WRITE_MAX_RETRIES - 1
    ) {
      const retryAfterMs = parseRetryAfterMs(response);
      const fallbackBackoff = HULY_WRITE_BASE_BACKOFF_MS * 2 ** attempt;
  ...
  return fetchWithPool(url, options);
}
...
export class HulyIssueClient {
  ...
  async listIssues(projectIdentifier, options = {}) {
    ...
    try {
      ...
      console.log(
        `[${this.name}] Fetched ${result.count} issues from ${projectIdentifier}${syncInfo} in ${executionTime}ms`
      );
  ...
  async listIssuesBulk(projectIdentifiers, options = {}) {
    ...
    try {
      ...
      console.log(
        `[${this.name}] Bulk fetched ${result.totalIssues} issues from ${result.projectCount} projects${syncInfo} in ${executionTime}ms`
      );
  ...
  async createIssue(projectIdentifier, issueData) {
    ...
    try {
      ...
      const response = await performWriteWithBackoff(url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  async updateIssue(issueIdentifier, field, value) {
    ...
    try {
      ...
      if (executionTime > 1000) {
        console.log(
          `[${this.name}] Updated issue ${issueIdentifier} ${field} in ${executionTime}ms`
        );
      }
  ...
  async deleteIssuesBulk(identifiers, options = {}) {
    ...
    try {
      ...
      const result = await response.json();
      console.log(
        `[${this.name}] Bulk deleted ${result.succeeded}/${identifiers.length} issues in ${executionTime}ms`
      );
  ...
  async getIssuesBulk(identifiers) {
    ...
    try {
      ...
      const result = await response.json();
      console.log(
        `[${this.name}] Bulk fetch returned ${result.issues?.length || 0} issues in ${executionTime}ms`
      );
  ...
  async searchIssuesGlobal(filters = {}) {
    ...
    try {
      ...
      const result = await response.json();
      console.log(
        `[${this.name}] Global search returned ${result.count || 0} issues in ${executionTime}ms`
      );
