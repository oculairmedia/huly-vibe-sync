  ...
  watchProject(projectIdentifier, projectPath) {
    ...
    try {
      const watcher = chokidar.watch(beadsPath, {
        ...
        // Ignore: hidden files (except .beads), node_modules, .git, and
        // temporary files created by beads atomic writes (issues.jsonl.tmp.*)
        // which cause EACCES when created by a different UID.
        ignored: [/(^|[\/\\])\..(?!beads)/, /node_modules/, /\.git/, /\.tmp/],
      });
