...
export class BeadsWatcher {
  ...
  watchProject(projectIdentifier, projectPath) {
    ...
    try {
      const watcher = chokidar.watch(beadsPath, {
        persistent: true,
        ignoreInitial: true,
        depth: 2, // Watch .beads and immediate subdirectories
        awaitWriteFinish: {
          stabilityThreshold: 500,
          pollInterval: 100,
        },
        ignored: [
          /(^|[\/\\])\../, // Ignore dotfiles (except .beads itself)
          /node_modules/,
          /\.git/,
        ],
      });
      ...
      watcher
        ...
        .on('change', filePath => this.handleChange(projectIdentifier, projectPath, filePath, 'change'))
        .on('unlink', filePath => this.handleChange(projectIdentifier, projectPath, filePath, 'unlink'))
        .on('error', error => {
          logger.error({ project: projectIdentifier, err: error }, 'Beads watcher error');
        })
        .on('ready', () => {
          logger.info(
            { project: projectIdentifier, beadsPath },
            'Beads watcher ready'
          );
        });
      ...
      logger.info(
        { project: projectIdentifier, beadsPath },
        'Started watching .beads directory'
      );
  ...
  handleChange(projectIdentifier, projectPath, filePath, eventType) {
    // Only care about issue files (JSON/YAML in .beads/issues/)
    const relativePath = path.relative(projectPath, filePath);
    
    // Skip non-issue files
    if (!relativePath.includes('.beads/issues') && !relativePath.includes('.beads\\issues')) {
      logger.debug(
  ...
  async triggerSync(projectIdentifier, projectPath) {
    ...
    logger.info(
      { project: projectIdentifier, fileCount: changedFiles.length, files: changedFiles.slice(0, 5) },
    ...
    if (this.onBeadsChange) {
      try {
        ...
        logger.error(
          ...
          'Error in Beads change callback'
        );
      }
    }
  }
  ...
  async syncWithDatabase() {
    ...
    logger.info(
      ...
      'Synced Beads watchers with database'
    );
