...
export class VibeEventWatcher {
   ...
   */
  constructor({ db, onTaskChange, vibeUrl, reconnectDelay = RECONNECT_DELAY }) {
    this.db = db;
  ...
  async connect() {
    ...
    try {
      ...
      this.connected = true;
      this.reconnectAttempts = 0;
      logger.info('✓ Connected to Vibe SSE stream');
  ...
  handleTaskPatch(patch) {
    ...
    logger.debug(
      { op, taskId, projectId, status: value?.status },
      'Vibe task change detected'
    );
