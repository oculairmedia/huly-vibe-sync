...
export class ProjectRegistry {
  ...
  /**
   * Read beads prefix from .beads/metadata.json or directory name.
   * Beads stores the project prefix in metadata.json as dolt_database
   * (with underscores), or we derive it from the directory name.
   * @param {string} dirPath
   * @returns {string|null}
   */
  _readBeadsPrefix(dirPath) {
    // Try metadata.json first (has dolt_database like "huly_vibe_sync")
    const metadataPath = path.join(dirPath, '.beads', 'metadata.json');
    try {
      if (fs.existsSync(metadataPath)) {
        const data = JSON.parse(fs.readFileSync(metadataPath, 'utf8'));
        if (data.dolt_database) {
          // Convert underscores to hyphens to match issue prefix convention
          return data.dolt_database.replace(/_/g, '-');
        }
      }
    } catch { /* fall through */ }

    // Fallback: use directory name (e.g. /opt/stacks/beads → "beads")
    return path.basename(dirPath);
  }

  /**
   * Count beads issues from issues.jsonl (one JSON object per line).
   * This is the flush file that beads maintains alongside its Dolt database.
   * Falls back to 0 if anything goes wrong.
   * @param {string} dirPath
   * @returns {number}
   */
  _countBeadsIssues(dirPath) {
    const jsonlPath = path.join(dirPath, '.beads', 'issues.jsonl');
    try {
