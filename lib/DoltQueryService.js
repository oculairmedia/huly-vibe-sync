...
export class DoltQueryService {
  constructor() {
    ...
    this._commitLogCache = { data: null, fetchedAt: 0 };

    /** Cache TTL in milliseconds (30 seconds) */
    this._commitLogCacheTTL = 30_000;
  }
  ...
  async getCommitLog(limit = 20) {
    ...
    // Return cached result if still fresh and limit fits
    const now = Date.now();
    if (
      this._commitLogCache.data &&
      now - this._commitLogCache.fetchedAt < this._commitLogCacheTTL &&
      this._commitLogCache.data.length >= limit
    ) {
      return this._commitLogCache.data.slice(0, limit);
    }
    ...
    // Cache the result
    this._commitLogCache = { data: rows, fetchedAt: now };
  ...
  /**
   * Get all active issues (non-tombstone, non-excluded status).
   * Used by reconciliation processes to sync full state.
   *
   * @param {string} excludeStatus - Status to exclude (default: 'tombstone')
   * @returns {Promise<Array<Object>>}
   */
  async getAllActiveIssues(excludeStatus = 'tombstone') {
    this._ensureConnected();
    const [rows] = await this.pool.execute(
      `SELECT id, title, status, priority, issue_type, description, created_at, updated_at
       FROM issues
       WHERE status != ?
  ...
  /**
   * Get all active issues with labels joined (for reconciliation).
   * Includes GROUP_CONCAT for labels to provide complete issue data.
   *
   * @param {string} excludeStatus - Status to exclude (default: 'tombstone')
   * @returns {Promise<Array<Object>>} Issues with labels as comma-separated string
   */
  async getAllActiveIssuesWithLabels(excludeStatus = 'tombstone') {
    this._ensureConnected();
    const [rows] = await this.pool.execute(
      `SELECT i.id, i.title, i.status, i.priority, i.issue_type, i.description,
              i.created_at, i.updated_at, GROUP_CONCAT(l.label) AS labels
       FROM issues i
       LEFT JOIN labels l ON i.id = l.issue_id
       WHERE i.status != ?
       GROUP BY i.id
