...
/**
 * Resolve Huly status for a Beads issue with metadata-aware fallback.
 *
 * Priority:
 * 1. `issue.metadata.huly_status` when present (except closed Beads issues)
 * 2. Classic Beads status mapping using labels
 *
 * If a Beads issue is closed, we always respect the close action and map from
 * Beads status (typically Done, or Canceled with huly:Canceled label).
 *
 * @param {Object} beadsIssue - Beads issue object
 * @returns {string} Effective Huly status
 */
export function getEffectiveHulyStatus(beadsIssue) {
  const issue = beadsIssue || {};
  const labels = Array.isArray(issue.labels) ? issue.labels : [];
  const storedStatus = issue?.metadata?.huly_status;
  const normalizedStoredStatus =
    typeof storedStatus === 'string' && storedStatus.trim().length > 0
      ? storedStatus.trim()
      : null;

  if (normalizedStoredStatus) {
    if (issue.status === 'closed') {
      return mapBeadsStatusToHuly('closed', labels);
    }
    return normalizedStoredStatus;
  }

  return mapBeadsStatusToHuly(issue.status, labels);
}
