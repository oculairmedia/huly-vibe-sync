...
/**
 * Map Huly status to Beads status
 * 
 * Beads has only two statuses: open, closed
 * 
 * @param {string} hulyStatus - The Huly status value
 * @returns {string} The mapped Beads status (open or closed)
 */
export function mapHulyStatusToBeads(hulyStatus) {
  ...
  // Closed statuses
  if (status.includes('done') || status.includes('completed') || status.includes('cancel')) {
    return 'closed';
  }
 ...
 * 
 * @param {string} beadsStatus - The Beads status (open or closed)
 * @returns {string} The mapped Huly status
 */
export function mapBeadsStatusToHuly(beadsStatus) {
  const statusMap = {
    open: 'Backlog',
    closed: 'Done',
  };
...
export function mapHulyPriorityToBeads(hulyPriority) {
  ...
  if (priority.includes('no') || priority.includes('none')) {
    return 5;
  }
 ...
 * 
 * @param {number} beadsPriority - The Beads priority (1-5)
