  ...
  async processPendingChanges(projectIdentifier) {
    ...
    try {
      ...
      if (entitiesToUpsert.length > 0) {
        ...
        const filePaths = entitiesToUpsert.map(e => e.name.replace(/^File:/, ''));
      ...
      if (this.astEnabled && entitiesToUpsert.length > 0) {
        const astResult = await this._processAstForFiles(
          projectIdentifier,
          projectPath,
          entitiesToUpsert.map(e => e.name.replace(/^File:/, '')),
          client
        );
    ...
    for (let i = 0; i < astFiles.length; i += BATCH_SIZE) {
      ...
      if (filesToSync.length > 0) {
        try {
          ...
          // Now sync functions and create edges
          const syncResult = await client.syncFilesWithFunctions({
            projectId: projectIdentifier,
            files: filesToSync,
            concurrency,
          ...
          result.filesProcessed += filesToSync.length;
          result.functionsSynced += syncResult.entities;
