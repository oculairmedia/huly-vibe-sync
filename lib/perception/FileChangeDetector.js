...
import { createGraphitiClient, createAstGraphitiClient } from '../../src/GraphitiClient.js';
...
export class FileChangeDetector {
  ...
  watchProject(projectIdentifier, projectPath) {
    ...
    // Create AST-namespaced client for module-level entities (separate group_id)
    const astClient = createAstGraphitiClient(this.config, projectIdentifier);
    if (astClient) {
      this._s.astGraphitiClients.set(projectIdentifier, astClient);
    }
    ...
    const watcher = /** @type {CodePerceptionWatcher} */ (chokidar.watch(projectPath, {
      ignored: watchedPath => this._s.shouldIgnorePath(watchedPath, projectPath),
  ...
  handleChange(projectIdentifier, filePath, changeType) {
    const watcher = this._s.watchers.get(projectIdentifier);
    const projectPath = watcher?._projectMeta?.projectPath || null;

    if (this._s.shouldIgnorePath(filePath, projectPath)) {
      return;
    }
