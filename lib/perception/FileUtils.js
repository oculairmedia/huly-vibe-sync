...
export function shouldIgnorePath(
  ...
  if (allowlistMode && sourceRoots.length > 0) {
    ...
    if (normalizedProjectPath) {
      if (normalizedPath === normalizedProjectPath) {
        relativePath = '';
      } else if (normalizedPath.startsWith(`${normalizedProjectPath}/`)) {
        relativePath = normalizedPath.slice(normalizedProjectPath.length + 1);
      }
    }
    ...
    const inAllowedRoot = sourceRoots.some(root => {
      ...
      return (
        ...
        normalizedRoot.startsWith(`${relativePath}/`)
      );
    });
    if (!inAllowedRoot) {
      return true;
  ...
  if (pathSegments.some(shouldIgnoreDir)) {
    return true;
  }
  ...
  for (const pattern of ignorePatterns) {
    try {
      ...
      // Ignore malformed custom patterns and continue evaluating others.
    }
  }
