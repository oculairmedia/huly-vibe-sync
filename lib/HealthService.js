...
export const metrics = {
  ...
  vibeApiLatency: new Histogram({
    name: 'vibe_api_latency_seconds',
    help: 'Vibe Kanban API call latency in seconds',
    labelNames: ['operation'], // listProjects, createProject, listTasks, etc.
    buckets: [0.1, 0.5, 1, 2, 5, 10],
    registers: [register],
  }),
...
export function recordApiLatency(service, operation, durationMs) {
  ...
  } else if (service === 'vibe') {
    metrics.vibeApiLatency.observe({ operation }, durationSeconds);
  }
}
