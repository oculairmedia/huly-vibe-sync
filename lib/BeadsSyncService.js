...
export async function syncBeadsIssueToHuly(
  ...
  if (!hulyIssue) {
    ...
    try {
      ...
      if (!hulyIssue) {
        console.warn(`[Beads→Huly] Huly issue ${hulyIdentifier} deleted - marking to skip future syncs`);
        db.markDeletedFromHuly(hulyIdentifier);
        return;
      }
    } catch (error) {
      console.warn(`[Beads→Huly] Huly issue ${hulyIdentifier} not found - marking to skip future syncs`);
      db.markDeletedFromHuly(hulyIdentifier);
      return;
    }
    } catch (error) {
