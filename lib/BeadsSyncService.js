...
// Import parent-child functions (will be moved to BeadsParentChildService later)
import {
...
export async function syncHulyIssueToBeads(projectPath, hulyIssue, beadsIssues, db, config = {}) {
  ...
  if (!beadsIssue) {
    // No mapping exists - check if a Beads issue with matching title exists
    const matchingBeadsIssue = findMatchingIssueByTitle(beadsIssues, hulyIssue.title);
  ...
  if (storedParentHulyId !== currentParentHulyId) {
    ...
    if (currentParentHulyId) {
      ...
      if (newParentDbIssue?.beads_issue_id) {
        ...
        db.updateParentChild(hulyIssue.identifier, currentParentHulyId, newParentDbIssue.beads_issue_id);
      } else {
...
export async function syncBeadsIssueToHuly(
  ...
  if (!dbIssue) {
    ...
    const matchingHulyIssue = findMatchingIssueByTitle(hulyIssues, beadsIssue.title);
  ...
  if (beadsStatusMapped !== hulyStatusNormalized) {
    ...
    const success = await updateHulyIssueStatus(hulyClient, hulyIdentifier, beadsStatusMapped, config);
  ...
  if (beadsIssue.title !== hulyIssue.title) {
    ...
    const success = await updateHulyIssueTitle(hulyClient, hulyIdentifier, beadsIssue.title, config);
  ...
  const currentParentBeadsId = await getBeadsParentId(projectPath, beadsIssue.id);
...
export async function batchSyncHulyToBeads(projectPath, hulyIssues, beadsIssues, db, config = {}) {
  ...
  for (const hulyIssue of hulyIssues) {
    try {
      const synced = await syncHulyIssueToBeads(projectPath, hulyIssue, beadsIssues, db, config);
      if (synced) {
        result.synced++;
        // Throttle after successful operations (they involve CLI calls)
...
export async function fullBidirectionalSync(
  ...
  results.hulyToBeads = await batchSyncHulyToBeads(projectPath, hulyIssues, beadsIssues, db, config);
