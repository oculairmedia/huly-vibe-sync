...
export async function syncBeadsIssueToHuly(
  ...
  const { mapBeadsStatusToHuly, mapBeadsPriorityToHuly, mapBeadsTypeToHuly } =
    await import('./statusMapper.js');
  ...
  if (!hulyIssue) {
    ...
    try {
      ...
      if (!hulyIssue) {
        console.warn(
          `[Beads→Huly] Huly issue ${hulyIdentifier} does not exist (may have been deleted)`
        );
        return;
      }
    } catch (error) {
      console.warn(`[Beads→Huly] Failed to fetch ${hulyIdentifier}: ${error.message}`);
      return;
    }
  }
...
export async function batchSyncBeadsToHuly(
  ...
  const dbIssues = db.getAllIssues();
  const beadsIdToDbIssue = new Map(dbIssues.filter(i => i.beads_issue_id).map(i => [i.beads_issue_id, i]));
  ...
  console.log(`[Beads→Huly] Pre-fetch analysis: ${beadsIssues.length} beads issues, ${hulyIssues.length} cached huly issues, ${dbIssues.length} db issues, ${missingIdentifiers.length} missing`);
  ...
  if (missingIdentifiers.length > 0) {
    ...
    try {
      ...
      for (let i = 0; i < missingIdentifiers.length; i += BATCH_SIZE) {
        ...
        console.log(`[Beads→Huly] Bulk fetched ${fetched.length}/${batch.length} issues (batch ${Math.floor(i/BATCH_SIZE) + 1})`);
      }
    } catch (error) {
      console.warn(`[Beads→Huly] Bulk pre-fetch failed, falling back to individual fetches: ${error.message}`);
    }
  }
