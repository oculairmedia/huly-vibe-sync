...
import { readIssuesFromJSONL } from './BeadsJSONLReader.js';
...
export function fetchAllIssues(projectPath) {
  ...
  if (beadsIssues.length === 0) {
    ...
    console.log('[BeadsDataFetcher] DB empty, trying JSONL reader');
    beadsIssues = readIssuesFromJSONL(projectPath);
  }

  if (beadsIssues.length === 0) {
    // Last resort: bd CLI
    console.log('[BeadsDataFetcher] JSONL empty, trying bd CLI');
