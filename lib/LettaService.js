...
export class LettaService {
  ...
  async syncToolsFromControl(agentId, forceSync = false) {
    ...
    console.log(`[Letta] Tool sync complete: ${result.attached} attached, ${result.detached} detached, ${result.skipped} already attached, ${result.errors.length} errors`);
  ...
  async ensureSource(sourceName, folderId = null) {
    ...
    try {
      ...
      if (error.message && error.message.includes('409')) {
        ...
        try {
          const response = await fetchWithPool(`${this.apiURL}/sources?name=${encodeURIComponent(sourceName)}&limit=10`, {
            method: 'GET',
            headers: {
              'Authorization': `Bearer ${this.password}`,
              'Content-Type': 'application/json',
          ...
          console.warn(`[Letta] ⚠️  Source ${sourceName} exists (409) but couldn't be found. Skipping upload.`);
