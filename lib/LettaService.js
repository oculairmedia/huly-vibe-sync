...
export class LettaService {
  constructor(baseURL, password, options = {}) {
    ...
    // Model: Claude Sonnet 4.5 (same as Meridian)
   ...
   * 
   * NOTE: MCP tool management is not yet supported in the current SDK version
   * TODO: Implement once SDK supports tools.mcp.* endpoints or via direct API calls
   * For now, MCP tools should be manually attached via Letta UI or API
   * 
  ...
  async attachMcpTools(agentId, hulyMcpUrl, vibeMcpUrl) {
    ...
    console.log(`[Letta] TODO: Manually attach Huly and Vibe MCP tools to agent ${agentId} via Letta UI`);
    return null;
    
    // DISABLED: SDK doesn't support tools.mcp yet
    // try {
