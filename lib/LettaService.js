    ...
    try {
      ...
      while (retries <= maxRetries) {
        try {
          ...
          const response = await fetchWithPool(`${this.baseURL}/v1/agents`, {
            method: 'POST',
            headers: {
      ...
      // Update persona block with our improved PM persona (since Letta creates default blocks)
      await this._updatePersonaBlock(agent.id, persona);
