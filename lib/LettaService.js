...
export class LettaService {
  ...
  updateAgentsMdWithProjectInfo(projectPath, agentId, projectInfo) {
    try {
      ...
      const marker = '<!-- HULY-PROJECT-INFO -->';

      // Generate the project info block
      const projectBlock = `${marker}
# Project Context

## Huly Integration
- **Project Code**: \`${projectInfo.identifier}\`
- **Project Name**: ${projectInfo.name}
- **Letta Agent ID**: \`${agentId}\`

## Project Agent Role
This project has an assigned **Letta PM Agent** (\`${agentId}\`) that acts as the senior developer and project manager. This agent:
...
1. **Before starting work**: Search Huly for related issues using \`huly-mcp\` with project code \`${projectInfo.identifier}\`
2. **Issue references**: All issues for this project use the format \`${projectInfo.identifier}-XXX\` (e.g., \`${projectInfo.identifier}-123\`)
...
\`\`\`json
{
  ...
  "agent": "${agentId}",
  "message": "Completed task ${projectInfo.identifier}-XXX: [summary of work done]"
}
\`\`\`

## Beads Issue Tracking

This project uses **three-way sync** between Huly, Vibe Kanban, and Beads (git-backed issue tracker).

### Quick Reference
\`\`\`bash
bd list                  # List all issues
bd show <id>             # View issue details  
bd create "Title" --priority=2 --type=task
bd update <id> --status in_progress
bd close <id>            # Mark complete
bd sync                  # Sync to git (auto-syncs to Huly via service)
\`\`\`

### Three-Way Sync Behavior
- **Create anywhere**: Issues created in Huly, Vibe, or Beads will sync to all three systems (~30-60s)
- **Updates sync bidirectionally**: Status, title, priority changes propagate automatically
- **Beads is git-backed**: Run \`bd sync\` then \`git push\` to persist changes
- **Dependencies planned**: blocks/blockedBy relationships coming soon

### When to Use Each System
- **Huly** (\`${projectInfo.identifier}-XXX\` issues): Web UI, detailed planning, epics, rich descriptions
- **Vibe Kanban**: AI agent task management, quick visual kanban board
- **Beads** (CLI): Local-first workflow, git integration, offline work, fast CLI operations

### Important Notes
- **Permission**: Ensure \`.beads/\` directory has correct ownership (user:1000 for containers)
- **Sync delay**: Changes take 30-60 seconds to propagate between systems
- **Title limits**: Keep issue titles under 100 chars and avoid special characters (×, quotes)
- **Git workflow**: Always \`bd sync && git push\` to persist beads changes
...
`;
      ...
      if (fs.existsSync(agentsMdPath)) {
        ...
        // Check if our block is already there
        if (existingContent.includes(marker)) {
          // Replace existing block
          const startMarker = marker;
          const endMarker = '<!-- END-HULY-PROJECT-INFO -->';
          const regex = new RegExp(
            `${startMarker}[\\s\\S]*?${endMarker}\\n*`,
            'g'
          );
          existingContent = existingContent.replace(regex, '');
          console.log(`[Letta] Updating existing project info in AGENTS.md`);
        }
      }

      // Prepend the project block
      const newContent = projectBlock + existingContent;
      fs.writeFileSync(agentsMdPath, newContent, { mode: 0o666 });
      console.log(`[Letta] ✓ Updated AGENTS.md with project info: ${agentsMdPath}`);
    } catch (error) {
  ...
  async ensureSearchFolderPassagesTool() {
    ...
    try {
      ...
      try {
        ...
        console.error(`[Letta] Could not read tool source from ${toolSourcePath}:`, readError.message);
      ...
      if (!options.docsOnly) {
        try {
          ...
          const sourceExtensions = [
            '.js', '.ts', '.tsx', '.jsx', '.mjs', '.cjs',  // JavaScript/TypeScript
            '.py', '.pyx',                                   // Python
            '.rs',                                           // Rust
            '.go',                                           // Go
            '.java', '.kt', '.scala',                        // JVM
...
export function buildComponentsSummary(components, hulyIssues) {
  ...
  const componentsList = Array.from(componentStats.values())
