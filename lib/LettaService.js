...
## Beads Issue Tracking (Hybrid Sync)
...
bd sync                  # Export to git (for persistence)
\`\`\`

### Hybrid Sync Behavior
- **Automatic**: \`bd create\`, \`bd update\`, \`bd close\` write to SQLite DB
- **Real-time**: File watcher detects DB changes, syncs to Huly (~30-60s)
- **Git backup**: \`bd sync\` exports to JSONL + git push (for cross-machine persistence)
...
- **Auto-sync**: Changes propagate to Huly automatically within ~30-60 seconds
- **Git backup**: Run \`bd sync\` before ending session for git persistence
- **Title limits**: Keep issue titles under 100 chars and avoid special characters
