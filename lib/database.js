...
import { SyncStateRepository } from './database/repositories/SyncStateRepository.js';
import {
...
const sqliteModuleName = globalThis.Bun ? 'bun:sqlite' : 'better-sqlite3';
...
export class SyncDatabase {
  constructor(dbPath) {
    ...
    this.projects = null;
    this.issues = null;
    this.metadata = null; // Backward-compatible alias to syncState
    this.syncHistory = null; // Backward-compatible alias to syncState
  ...
  createTables() {
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS projects (
        ...
        beads_remote_owner TEXT,
        beads_remote_repo TEXT,
        beads_remote_url TEXT,
        beads_remote_name TEXT,
        beads_remote_status TEXT,
        beads_remote_visibility TEXT,
        beads_remote_provisioned_at INTEGER,
        beads_remote_last_push_at INTEGER,
  ...
  updateProject(identifier, updates) {
    return this.projects.updateProject(identifier, updates);
  }

  archiveProject(identifier) {
    return this.projects.archiveProject(identifier);
  }

  unarchiveProject(identifier) {
    return this.projects.unarchiveProject(identifier);
  }

  deleteProject(identifier) {
    return this.projects.deleteProject(identifier);
  }
  ...
  getIssueByVibeTaskId(projectIdentifier, vibeTaskId) {
    return this.issues.getIssueByVibeTaskId(projectIdentifier, vibeTaskId);
  }
   ...
   */
  updateParentChild(identifier, parentHulyId) {
    this.issues.updateParentChild(identifier, parentHulyId);
  }
   ...
   */
  startSyncRun() {
    return this.syncState.startSyncRun();
  }
  ...
  completeSyncRun(syncId, stats) {
    this.syncState.completeSyncRun(syncId, stats);
  }
  ...
  getRecentSyncs(limit = 10) {
    return this.syncState.getRecentSyncs(limit);
  }
   ...
   */
  getStats() {
    return this.projects.getStats(this.getLastSync());
  }
