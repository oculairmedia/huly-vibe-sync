...
export class SyncDatabase {
  ...
  upsertProject(project) {
    ...
    const stmt = this.db.prepare(`
      INSERT INTO projects (
        ...
        issue_count, last_checked_at, last_sync_at, status, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(identifier) DO UPDATE SET
        name = excluded.name,
        huly_id = COALESCE(excluded.huly_id, huly_id),
        vibe_id = COALESCE(excluded.vibe_id, vibe_id),
        filesystem_path = COALESCE(excluded.filesystem_path, filesystem_path),
   ...
   */
  getProjectsToSync(cacheExpiryMs = 300000) {
    const cutoff = Date.now() - cacheExpiryMs;
    return this.db.prepare(`
      SELECT * FROM projects
      WHERE status = 'active'
        AND (issue_count > 0 OR last_checked_at < ?)
  ...
  getProjectIssues(projectIdentifier) {
    return this.db.prepare(
      'SELECT * FROM issues WHERE project_identifier = ? ORDER BY identifier'
  ...
  getStats() {
    ...
    const activeProjects = this.db.prepare('SELECT COUNT(*) as count FROM projects WHERE issue_count > 0').get().count;
  ...
  getProjectSummary() {
    return this.db.prepare(`
      SELECT identifier, name, issue_count, last_sync_at, last_checked_at
      FROM projects
      ORDER BY issue_count DESC, name
    `).all();
  }
