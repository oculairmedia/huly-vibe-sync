...
export class SyncDatabase {
  ...
  createTables() {
    ...
    this.migrateBookStackTables();
    this.migrateDeletionColumns();
  }
  ...
  /**
   * Add deletion tracking columns to issues table (migration)
   */
  migrateDeletionColumns() {
    const columns = this.db.prepare('PRAGMA table_info(issues)').all();
    const columnNames = columns.map(c => c.name);

    if (!columnNames.includes('deleted_from_vibe')) {
      console.log('[DB] Adding deleted_from_vibe column to issues table');
      this.db.exec(`ALTER TABLE issues ADD COLUMN deleted_from_vibe INTEGER DEFAULT 0`);
    }

    if (!columnNames.includes('deleted_from_beads')) {
      console.log('[DB] Adding deleted_from_beads column to issues table');
      this.db.exec(`ALTER TABLE issues ADD COLUMN deleted_from_beads INTEGER DEFAULT 0`);
    }
  ...
  migrateBookStackTables() {
    ...
    if (tables.length > 0) {
      ...
      if (fkCheck?.sql?.includes('FOREIGN KEY')) {
        ...
        this.db.exec('DROP TABLE bookstack_pages');
      }
    }

    const tablesAfter = this.db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='bookstack_pages'")
      .all();
    ...
    if (!colNames.includes('bookstack_last_export_at')) {
      ...
      this.db.exec('ALTER TABLE projects ADD COLUMN bookstack_last_export_at INTEGER');
    }
    if (!colNames.includes('bookstack_book_slug')) {
      console.log('[DB] Adding bookstack_book_slug column to projects table');
      this.db.exec('ALTER TABLE projects ADD COLUMN bookstack_book_slug TEXT');
    }

    // Phase 3: bidirectional sync columns
    const bsColumns = this.db.prepare('PRAGMA table_info(bookstack_pages)').all();
    ...
    if (!bsColNames.includes('bookstack_content_hash')) {
      ...
      this.db.exec('ALTER TABLE bookstack_pages ADD COLUMN bookstack_content_hash TEXT');
    }
    if (!bsColNames.includes('sync_status')) {
      console.log('[DB] Adding sync_status column to bookstack_pages table');
      this.db.exec("ALTER TABLE bookstack_pages ADD COLUMN sync_status TEXT DEFAULT 'synced'");
    }
    if (!bsColNames.includes('bookstack_revision_count')) {
      ...
      this.db.exec('ALTER TABLE bookstack_pages ADD COLUMN bookstack_revision_count INTEGER');
    }
  }
  ...
  markDeletedFromVibe(identifier) {
    const stmt = this.db.prepare(`
      UPDATE issues
      SET deleted_from_vibe = 1, vibe_task_id = NULL, updated_at = ?
      WHERE identifier = ?
    `);
    return stmt.run(Date.now(), identifier);
  }
  ...
  getIssuesWithVibeTaskId(projectIdentifier = null) {
    if (projectIdentifier) {
      return this.db
        ...
        WHERE project_identifier = ?
          AND vibe_task_id IS NOT NULL
          AND deleted_from_vibe = 0
        ORDER BY identifier
      `
        )
        .all(projectIdentifier);
    ...
    return this.db
      .prepare(
        `
      SELECT * FROM issues
      WHERE vibe_task_id IS NOT NULL
        AND deleted_from_vibe = 0
      ORDER BY identifier
    `
      )
  ...
  getProjectByFolderName(folderNameOrPath) {
    ...
    // Normalize: extract folder name from path, lowercase
    ...
    // Try folder name match (extract folder from stored paths)
    const allProjects = this.db
      .prepare(
        `
      SELECT identifier, name, filesystem_path
  ...
  resolveProjectIdentifier(projectIdOrFolder) {
    ...
    // First, check if it's already a valid project identifier
