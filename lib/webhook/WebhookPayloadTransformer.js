...
/**
 * Extract change-watcher events from supported payload envelopes.
 *
 * Supported:
 * - { events: [...] }
 * - { type: 'notify', data: { events: [...] } }
 *
 * @param {Object} payload
 * @returns {Array}
 */
export function getChangeWatcherEvents(payload) {
  if (Array.isArray(payload?.events)) {
    return payload.events;
  }

  if (payload?.type === 'notify' && Array.isArray(payload?.data?.events)) {
...
export function isChangeWatcherFormat(payload) {
  return (
    ...
    (payload?.type === 'notify' && Array.isArray(payload?.data?.events))
  );
}
...
export function transformChangeWatcherEvents(events) {
  ...
  const issueTaskChanges = transformed.filter(
    c =>
      c._eventType === 'issue.updated' ||
      c._eventType === 'task.updated' ||
      c._eventType === 'issue.created' ||
      c._eventType === 'task.created'
  );
  const projectChanges = transformed.filter(
    c => c._eventType === 'project.updated' || c._eventType === 'project.created'
  );
