...
export class WebhookDebouncer {
   ...
   * @param {number} [options.maxBufferSize=500] - Hard cap before immediate flush
  ...
  buffer(changes) {
    ...
    if (this._pendingChanges.length >= this._maxBufferSize) {
      ...
      logger.warn(
        {
          buffered: this._pendingChanges.length,
          maxBufferSize: this._maxBufferSize,
        },
        '[WebhookDebouncer] Buffer cap reached, flushing immediately'
      );
  ...
  async flush() {
    ...
    this._flushPromise = (async () => {
      try {
        ...
        for (let i = 0; i < deduped.length; i += this._maxBatchSize) {
          const chunk = deduped.slice(i, i + this._maxBatchSize);
          const changesByProject = this._groupChangesByProject(chunk);
          ...
          logger.info(
            { workflowId, changeCount: chunk.length, chunkStart: i },
            '[WebhookDebouncer] Flushed debounced chunk to Temporal'
          );
        }
      } catch (error) {
