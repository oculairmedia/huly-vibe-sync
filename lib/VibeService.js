...
import { determineGitRepoPath, validateGitRepoPath } from './textParsers.js';
...
export async function createVibeProject(vibeClient, hulyProject, config = {}) {
  ...
  try {
    ...
    if (existingProject) {
      console.log(
        `[Vibe] ✓ Found existing project: ${hulyProject.name} (ID: ${existingProject.id}) - skipping creation`
      );
    ...
    if (!validation.valid) {
      console.warn(
        `[Vibe] ⚠ Skipping project ${hulyProject.name}: invalid repo path — ${validation.reason}`
      );
      recordApiLatency('vibe', 'createProject', Date.now() - startTime);
      return null;
    }
...
/**
 * Resolve parent issue identifier from Huly issue shape variants.
 * Supports object form ({ identifier }) and legacy string form.
 */
function resolveParentIdentifier(hulyIssue) {
...
/**
 * Build a Vibe description with sync metadata footer.
 * Includes explicit parent metadata so top-level moves can be represented.
 */
function buildVibeDescriptionFromHulyIssue(hulyIssue) {
  const parentIdentifier = resolveParentIdentifier(hulyIssue);
  const footerLines = [
    `Huly Issue: ${hulyIssue.identifier}`,
    `Huly Parent: ${parentIdentifier || 'none'}`,
  ];

  if (hulyIssue.description) {
    return `${hulyIssue.description}\n\n---\n${footerLines.join('\n')}`;
  }

  return `Synced from Huly: ${hulyIssue.identifier}\n\n---\n${footerLines.join('\n')}`;
}
  ...
  try {
    ...
    const byTitle = tasks.find(task => {
      ...
      if (normalizedHulyTitle.length > 15 && normalizedTaskTitle.length > 15) {
        if (
          normalizedTaskTitle.includes(normalizedHulyTitle) ||
...
export async function createVibeTask(vibeClient, vibeProjectId, hulyIssue, config = {}) {
  ...
  try {
    ...
    if (existingTask) {
      console.log(
        `[Vibe] ✓ Found existing task for ${hulyIssue.identifier}: "${existingTask.title}" (ID: ${existingTask.id}) - skipping creation`
      );
    ...
    // Add Huly issue + parent metadata for round-trip parent-child sync.
    const description = buildVibeDescriptionFromHulyIssue(hulyIssue);
