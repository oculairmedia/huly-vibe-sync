...
export async function listBeadsIssues(projectPath, filters = {}) {
  ...
  try {
    let issues = readIssuesFromDB(projectPath);
...
// updateBeadsIssueStatusWithLabel removed — depended on deleted statusMapper.js (Huly-specific)
...
export async function deleteBeadsIssue(projectPath, issueId, config = {}) {
  if (config.sync?.dryRun) {
    console.log(`[Beads] [DRY RUN] Would delete issue ${issueId}`);
    return true;
  }

  try {
    await execBeadsCommand(`delete ${shellQuote(issueId)} --yes`, projectPath);
    return true;
  } catch (error) {
    console.error(`[Beads] Error deleting issue ${issueId}:`, error.message);
    return false;
  }
}
