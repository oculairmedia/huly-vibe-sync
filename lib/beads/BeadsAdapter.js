...
export class BeadsAdapter {
  ...
  _defaultRunCommand(command, args = []) {
    ...
    try {
      ...
      throw new Error(`Beads command failed: ${displayCommand}\n${error.message}`);
    }
  }

  _extractBeadsDirArg(args) {
    const prefix = '--beads-dir=';
    const beadsDirArg = args.find((arg) => typeof arg === 'string' && arg.startsWith(prefix));
    return {
  ...
  _addProjectBeadsDir(args, project) {
