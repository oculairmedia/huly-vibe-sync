...
export async function batchSyncHulyToBeads(projectPath, hulyIssues, beadsIssues, db, config = {}) {
  ...
  if (!config?.sync?.skipRepoValidation) {
    const validation = validateGitRepoPath(projectPath);
    if (!validation.valid) {
      console.warn(
        `[Sync] Skipping Huly→Beads sync due to invalid repo path "${projectPath}": ${validation.reason}`
      );
      return {
        ...result,
        skipped: hulyIssues.length,
        errorMessages: [`Invalid repo path: ${validation.reason}`],
      };
    }
    console.log(`[Sync] Validated repository path for Huly→Beads: ${projectPath}`);
  }
...
export async function batchSyncBeadsToHuly(
  ...
  if (!config?.sync?.skipRepoValidation) {
    ...
    if (!validation.valid) {
      console.warn(
        `[Sync] Skipping Beads→Huly sync due to invalid repo path "${projectPath}": ${validation.reason}`
      );
      return {
        ...result,
        skipped: beadsIssues.length,
        errorMessages: [`Invalid repo path: ${validation.reason}`],
      };
    }
    console.log(`[Sync] Validated repository path for Beads→Huly: ${projectPath}`);
  }
  ...
  const configWithParentMap = {
    ...config,
    parentMap: lookups.parentMap,
    lookupMaps: { hulyIssueMap, beadsIdToDbIssue },
  };
...
export async function fullBidirectionalSync(
  ...
  if (!validation.valid) {
    console.warn(
      `[Sync] Skipping ${projectIdentifier} Beads sync due to invalid repo path "${projectPath}": ${validation.reason}`
    );
    results.hulyToBeads = {
      synced: 0,
      skipped: hulyIssues.length,
  ...
  console.log(`[Sync] Validated repository path for ${projectIdentifier}: ${projectPath}`);
  ...
  if (!validatedConfig.sync?.dryRun) {
