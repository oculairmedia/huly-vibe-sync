...
export async function syncBeadsParentChildToHuly(
  ...
  try {
    ...
    const beadsIssueIndex =
      existingBeadsIssueIndex ??
      (typeof db.getProjectIssues === 'function'
        ? buildIssueIndexByBeadsId(db.getProjectIssues(projectIdentifier))
...
export async function syncParentChildToHuly(
  ...
  const index =
    ...
    (typeof db.getAllIssues === 'function'
      ? buildIssueIndexByBeadsId(db.getAllIssues())
      : new Map());
  const childDbIssue = index.get(childBeadsIssue.id);
  const parentDbIssue = index.get(parentBeadsIssue.id);
...
export async function syncAllParentChildToHuly(
  ...
  const beadsIssueById = new Map(beadsIssues.map(i => [i.id, i]));
  ...
  for (const childIssue of issuesWithDeps) {
    try {
      ...
      for (const node of tree) {
        if (node.depth === 1) {
          const parentIssue = beadsIssueById.get(node.id);
          if (parentIssue) {
            const success = await syncParentChildToHuly(
              hulyClient, projectPath, childIssue, parentIssue, db, config, sharedIndex
            );
