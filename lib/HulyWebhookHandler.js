...
export class HulyWebhookHandler {
  ...
  async handleTaskChanges(changes, result) {
    ...
    if (temporal) {
      this._pendingChanges.push(...relevantChanges);

      if (this._debounceTimer) {
        clearTimeout(this._debounceTimer);
      }

      this._debounceTimer = setTimeout(() => this._flushPendingChanges(), this._debounceWindowMs);

      logger.info(
        { buffered: this._pendingChanges.length, added: relevantChanges.length },
        '[HulyWebhookHandler] Buffered changes, flush in ' + this._debounceWindowMs + 'ms'
      );
  ...
  async _flushPendingChanges() {
    ...
    this._flushPromise = (async () => {
      try {
        ...
        const changesByProject = this.groupChangesByProject(deduped);

        logger.info(
          { raw: changesToFlush.length, deduped: deduped.length },
          '[HulyWebhookHandler] Flushing debounced changes'
        );

        if (deduped.length > 50) {
          logger.warn(
            { changeCount: deduped.length },
            '[HulyWebhookHandler] Debounced batch still oversized, skipping'
          );
          return;
        }
        ...
        logger.info(
          ...
          '[HulyWebhookHandler] Flushed debounced changes to Temporal'
        );
      } catch (error) {
        logger.error({ err: error }, '[HulyWebhookHandler] Debounce flush failed');
        this.stats.errors++;
      } finally {
        this._flushPromise = null;
      }
