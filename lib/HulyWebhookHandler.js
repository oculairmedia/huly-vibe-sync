...
import { WebhookDebouncer } from './webhook/WebhookDebouncer.js';
import {
  isChangeWatcherFormat,
  getChangeWatcherEvents,
  transformChangeWatcherEvents,
  groupChangesByProject,
  filterRelevantChanges,
} from './webhook/WebhookPayloadTransformer.js';
...
export class HulyWebhookHandler {
  ...
  constructor({ db, onChangesReceived, changeWatcherUrl, callbackUrl }) {
    ...
    this._debouncer = new WebhookDebouncer({
  ...
  async handleWebhook(payload) {
    ...
    if (isChangeWatcherFormat(payload)) {
  ...
  async handleChangeWatcherPayload(payload, result) {
    const events = getChangeWatcherEvents(payload);
    const payloadSource = payload.source || payload.data?.source || 'unknown';
    const payloadTimestamp = payload.timestamp || payload.data?.timestamp;
    ...
    // Transform and separate changes using the payload transformer
    const { issueTaskChanges, projectChanges } = transformChangeWatcherEvents(events);
  ...
  async handleTaskChanges(changes, result) {
    ...
    const { issueChanges, projectChanges: projChanges, relevantChanges } = filterRelevantChanges(changes);
    ...
    if (temporal) {
      this._debouncer.buffer(relevantChanges);
  ...
  getStats() {
    return {
      ...
      ...this.stats,
      debounceFlushes: this._debouncer.stats.debounceFlushes,
    };
  }
}
