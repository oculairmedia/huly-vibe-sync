...
export class LettaAgentLifecycleService {
  ...
  /**
   * Build a description string for a PM agent from its project name.
   * @param {string} projectName - Human-readable project name
   * @returns {string}
   */
  static buildAgentDescription(projectName) {
    return `PM agent for ${projectName} — manages issues, syncs code context, and coordinates development tasks.`;
  }

  /**
   * Backfill agent description if missing. Called on resume/ensure paths
   * so existing agents get descriptions without manual intervention.
   */
  async _ensureDescription(agent, projectName) {
    if (agent.description) return;
    const description = LettaAgentLifecycleService.buildAgentDescription(projectName);
    try {
      await this.config.client.agents.modify(agent.id, { description });
      console.log(`[Letta] ✓ Backfilled description for ${agent.name}`);
    } catch (error) {
      console.warn(
        `[Letta] ⚠️  Failed to backfill description for ${agent.name}: ${error.message}`
      );
    }
  }
  ...
  async ensureAgent(projectIdentifier, projectName) {
    ...
    try {
      console.log(
        `[Letta] Querying Letta for agents with name: ${agentName}, tags: vibesync, project:${projectIdentifier}`
      );
      ...
      queryParams.append('tags', 'vibesync');
      ...
      const allAgents = /** @type {Array<any>} */ (await response.json());
      ...
      if (pmAgents && pmAgents.length > 0) {
        if (pmAgents.length > 1) {
          ...
          await this._ensureDescription(existingAgent, projectName);
      ...
      while (retries <= maxRetries) {
        try {
          const createResp = await fetchWithPool(`${apiURL}/agents`, {
            ...
            body: JSON.stringify({
              name: agentName,
              description: LettaAgentLifecycleService.buildAgentDescription(projectName),
              agent_type: 'letta_v1_agent',
              model,
              embedding,
          ...
          if (isRateLimit && retries < maxRetries) {
            ...
            checkParams.append('tags', 'vibesync');
            ...
            if (checkResp.ok) {
              const existingAgents = /** @type {Array<any>} */ (await checkResp.json());
