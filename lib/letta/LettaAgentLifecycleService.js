...
export class LettaAgentLifecycleService {
  ...
  async ensureAgent(projectIdentifier, projectName) {
    ...
    try {
      console.log(
        `[Letta] Querying Letta for agents with name: ${agentName}, tags: vibe-sync, project:${projectIdentifier}`
      );
      ...
      queryParams.append('tags', 'vibe-sync');
      ...
      while (retries <= maxRetries) {
        try {
          ...
          if (isRateLimit && retries < maxRetries) {
            ...
            checkParams.append('tags', 'vibe-sync');
