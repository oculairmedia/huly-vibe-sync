...
export class LettaToolService {
  ...
  async attachMcpTools(agentId) {
    console.warn(
      '[Letta] attachMcpTools() is deprecated; syncing PM tools from the control agent instead. Issue operations now use Beads (`bd`), not Project MCP.',
    );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (Array.isArray(tools) && tools.length > 0) {
          ...
          if (firstTool && typeof firstTool === 'object' && 'id' in firstTool) {
            const toolId = String(firstTool.id);
            console.log(`[Letta] search_folder_passages tool already exists: ${toolId}`);
            return toolId;
          }
        }
      }
      ...
      if (!newTool || typeof newTool !== 'object' || !('id' in newTool)) {
        throw new Error('Failed to create tool: response did not include an id');
      }

      const newToolId = String(newTool.id);
      console.log(`[Letta] ✓ search_folder_passages tool created: ${newToolId}`);
      return newToolId;
    } catch (error) {
