...
export class LettaToolService {
  ...
  async attachMcpTools(agentId, projectMcpUrl) {
    const result = await this.attachPmTools(agentId);

    // Register Project MCP endpoint if URL provided
    if (projectMcpUrl) {
      try {
        console.log(`[Letta] Registering Project MCP endpoint: ${projectMcpUrl}`);
        const mcpTool = await this._ensureMcpTool('project-mcp', projectMcpUrl);
        if (mcpTool && mcpTool.id) {
          try {
            await this.config.client.agents.tools.attach(agentId, mcpTool.id);
            console.log(`[Letta] ✓ Project MCP tools attached to agent ${agentId}`);
          } catch (attachError) {
            if (attachError.message && attachError.message.includes('already attached')) {
              console.log(`[Letta] Project MCP tools already attached to agent ${agentId}`);
            } else {
              console.error(`[Letta] Error attaching Project MCP tool:`, attachError.message);
            }
          }
        }
      } catch (mcpError) {
        console.error(`[Letta] Failed to register Project MCP endpoint:`, mcpError.message);
        // Non-fatal — PM tools are still attached via attachPmTools()
      }
    }
