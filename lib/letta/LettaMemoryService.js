...
/**
 * Role overrides for agents that aren't standard PM agents.
 * Agents not listed here default to 'pm'.
 */
const AGENT_ROLE_OVERRIDES = {
  'Kitchen': 'companion',
  'Incognito': 'developer',
  'Memo': 'developer',
  'letta-code-agent': 'developer',
};
...
export class LettaMemoryService {
  ...
  async _ensureTemplateBlocks(agentId, { agentName } = {}) {
    try {
      const role = (agentName && AGENT_ROLE_OVERRIDES[agentName]) || 'pm';
      const templateBlocks = [{ label: 'expression', value: buildExpression(role) }];
