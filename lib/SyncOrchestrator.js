  ...
  try {
    ...
    const processProject = async hulyProject => {
      ...
      if (config.beads?.enabled) {
        const { listBeadsIssues, syncHulyIssueToBeads, syncBeadsIssueToHuly, ensureBeadsInitialized } = await import(
          './BeadsService.js'
        );
        ...
        if (gitRepoPath) {
          ...
          // Track which beads issues were updated in Phase 3
          ...
          log.info(
            { count: beadsIssues.length, phase: '3b', project: projectIdentifier },
