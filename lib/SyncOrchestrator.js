...
async function syncVibeTaskToHuly(
  ...
  const vibeDescWithoutFooter =
    vibeTask.description?.replace(/\n\n---\nHuly Issue: [A-Z]+-\d+$/, '') || '';
  ...
  if (vibeDescWithoutFooter !== hulyDesc) {
    ...
    if (!hulyDescChanged) {
      log.info(
        { identifier: hulyIdentifier, title: vibeTask.title },
        'Vibe→Huly: Updating description'
      );
  ...
  if (vibeStatusMapped !== hulyStatusNormalized) {
    ...
    if (hulyChanged) {
      log.debug(
        ...
        'Skipping Phase 2 - Huly changed, letting Phase 1 handle it'
      );
      return;
    }
        ...
        if (!existingTask) {
          ...
          if (fullHulyDescription !== existingTask.description) {
            const hulyDescChanged =
              lastKnownHulyDesc && hulyIssue.description !== lastKnownHulyDesc;
