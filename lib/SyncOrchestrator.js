  ...
  try {
    ...
    const processProject = async hulyProject => {
      ...
      if (bulkIssuesData?.projects?.[projectIdentifier]) {
        ...
        syncMeta = projectData.syncMeta || { latestModified: null, serverTime: new Date().toISOString() };
      } else {
        ...
        if (gitRepoPath) {
          ...
          if (!isInitialized) {
            ...
            if (missingIdentifiers.length > 0) {
              log.info({ count: missingIdentifiers.length, phase: '3b' }, 'Pre-fetching missing issues in bulk');
              try {
                const BATCH_SIZE = 50;
                for (let i = 0; i < missingIdentifiers.length; i += BATCH_SIZE) {
                  ...
                  log.info({ fetched: fetched.length, batch: Math.floor(i/BATCH_SIZE) + 1 }, 'Bulk fetched issues');
                }
              } catch (error) {
                log.warn({ error: error.message }, 'Bulk pre-fetch failed, falling back to individual fetches');
              }
            }
