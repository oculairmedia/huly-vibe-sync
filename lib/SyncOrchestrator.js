...
export async function syncHulyToVibe(
  ...
  try {
    ...
    const processProject = async hulyProject => {
      ...
      // PHASE 4: BookStack ↔ Filesystem (documentation sync - export only in Phase 1)
      ...
      // PHASE 4b: Local → BookStack import (opt-in via BOOKSTACK_IMPORT_ON_SYNC)
      if (config.bookstack?.enabled && config.bookstack?.importOnSync && bookstackService) {
        const filesystemPath4b = determineGitRepoPath(hulyProject);
        if (filesystemPath4b) {
          try {
