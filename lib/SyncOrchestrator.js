    ...
    const processProject = async (hulyProject) => {
      ...
      if (lettaService && !config.sync.dryRun) {
        try {
          ...
          await lettaService.updateProjectMemory(
            projectIdentifier,
            buildProjectMeta(hulyProject, vibeProject, gitRepoPath, hulyIssues, vibeTasks),
            buildBoardConfig(),
            buildBoardMetrics(hulyIssues, vibeTasks),
            buildHotspots(hulyIssues, vibeTasks),
            buildBacklogSummary(hulyIssues, vibeTasks)
          );
        } catch (lettaError) {
