 ...
 *
 * Deprecated: Use Temporal workflows instead of SyncOrchestrator.
 * The legacy sync path will be removed after migration.
 */
...
import { mapHulyStatusToVibe, mapVibeStatusToHuly, normalizeStatus } from './statusMapper.js';
import { extractHulyIdentifier, determineGitRepoPath, resolveGitUrl } from './textParsers.js';
import { processBatch } from './utils.js';
import { createSyncLogger } from './logger.js';
import { recordSyncStats } from './HealthService.js';
import { computeIssueContentHash } from './database/utils.js';
...
let temporalLettaClient = null;
const USE_TEMPORAL_SYNC = process.env.USE_TEMPORAL_SYNC === 'true';
const USE_TEMPORAL_LETTA = process.env.USE_TEMPORAL_LETTA === 'true';
let legacyDeprecationWarned = false;

function warnLegacyUsage() {
  if (legacyDeprecationWarned) return;
  legacyDeprecationWarned = true;
  console.warn('[SyncOrchestrator] Deprecated: use Temporal workflows instead of SyncOrchestrator');
}
...
async function getTemporalClient() {
  if (!temporalClient && USE_TEMPORAL_SYNC) {
    try {
      const { scheduleSingleIssueSync, scheduleVibeToHulySync, isTemporalAvailable } = await import(
...
/**
 * Get Temporal Letta client (lazy initialization)
 */
async function getTemporalLettaClient() {
  if (!temporalLettaClient && USE_TEMPORAL_LETTA) {
    try {
      const { provisionSingleAgent, isTemporalAvailable } = await import(
        '../temporal/dist/client.js'
      );

      if (await isTemporalAvailable()) {
        temporalLettaClient = { provisionSingleAgent };
        console.log('[SyncOrchestrator] Temporal Letta provisioning enabled');
      } else {
        console.warn(
          '[SyncOrchestrator] Temporal not available, falling back to direct Letta provisioning'
        );
      }
    } catch (err) {
      console.warn('[SyncOrchestrator] Failed to load Temporal Letta client:', err.message);
    }
  }
  return temporalLettaClient;
}

/**
 * Ensure a Letta agent exists for a project (with optional Temporal workflow)
 *
 * When USE_TEMPORAL_LETTA=true: Executes ProvisionSingleAgentWorkflow for durable provisioning
 * When USE_TEMPORAL_LETTA=false: Uses direct lettaService.ensureAgent()
 *
 * @param {Object} lettaService - Letta service instance
 * @param {string} projectIdentifier - Huly project identifier
 * @param {string} projectName - Huly project name
 * @returns {Promise<{id: string, temporal: boolean}>}
 */
async function ensureAgentWithFallback(lettaService, projectIdentifier, projectName) {
  const temporal = await getTemporalLettaClient();

  if (temporal) {
    try {
      const result = await temporal.provisionSingleAgent({
        projectIdentifier,
        projectName,
        attachTools: true,
      });
      ...
      if (result.success && result.agentId) {
        console.log(
          `[SyncOrchestrator] Temporal provisioned agent ${result.agentId} for ${projectIdentifier} (created=${result.created})`
        );
        return { id: result.agentId, temporal: true };
      }

      // Temporal returned failure — fall through to direct provisioning
      console.warn(
        `[SyncOrchestrator] Temporal provisioning failed for ${projectIdentifier}: ${result.error || 'unknown'}, falling back`
      );
    } catch (err) {
      console.warn(
        `[SyncOrchestrator] Temporal Letta failed, falling back to direct: ${err.message}`
      );
    }
  }

  // Direct provisioning fallback
  const agent = await lettaService.ensureAgent(projectIdentifier, projectName);
  return { id: agent.id, temporal: false };
}
...
export async function syncHulyToVibe(
  ...
  try {
    ...
    const processProject = async hulyProject => {
      ...
      if (lettaService && !config.sync.dryRun) {
        try {
          ...
          if (!lettaInfo || !lettaInfo.letta_agent_id) {
            ...
            const agent = await ensureAgentWithFallback(
              lettaService,
              projectIdentifier,
              hulyProject.name
            );
          ...
          if (lettaInfo && lettaInfo.letta_agent_id) {
            ...
            const memoryBlocks = [
              {
                label: 'project',
                value: buildProjectMeta(hulyProject, vibeProject, gitRepoPath, null),
              },
              ...
              { label: 'expression', value: buildExpression('pm') },
            ];
      ...
      const processPhase1Issue = async hulyIssue => {
        ...
        if (!existingTask) {
          ...
          // Skip unchanged: if Huly content hash matches stored AND Vibe status is stable
          if (dbIssue?.content_hash) {
            const newHash = computeIssueContentHash(hulyIssue);
            const vibeStatusStable =
              normalizeStatus(existingTask.status) === normalizeStatus(dbIssue.vibe_status);

            if (newHash === dbIssue.content_hash && vibeStatusStable) {
              phase1Skipped++;
              return;
            }
          }
      ...
      if (hulyIssues.length > 0) {
        await processBatch(hulyIssues, config.sync.maxWorkers, processPhase1Issue);
      }

      phaseTimings.phase1 = Date.now() - phase1Start;
      phaseCounts.phase1 = {
        total: hulyIssues.length,
        skipped: phase1Skipped,
        updated: phase1UpdatedTasks.size,
      };
      ...
      if (phase1Skipped > 0) {
        log.info(
          ...
          'Phase 1: Skipped unchanged issues'
        );
      }
      ...
      const processPhase2Task = async vibeTask => {
      ...
      if (vibeTasks.length > 0) {
        await processBatch(vibeTasks, config.sync.maxWorkers, processPhase2Task);
      }

      phaseTimings.phase2 = Date.now() - phase2Start;
      phaseCounts.phase2 = { total: vibeTasks.length };
      ...
      if (config.beads?.enabled) {
        ...
        if (gitRepoPath) {
          ...
          if (!isInitialized) {
            ...
            // Fetch beads issues and build O(1) lookup maps for dedup
            const beadsIssues = await listBeadsIssues(gitRepoPath);
            const beadsLookups = buildIssueLookups(beadsIssues);
            ...
            // Phase 3a runs sequentially (concurrency=1) because:
            // - Beads CLI writes to same git repo (inherently serial I/O)
            // - Prevents race where concurrent workers both create for the same title match
            const beadsConfig = { ...config, lookups: beadsLookups };
            const processPhase3aIssue = async hulyIssue => {
              ...
              if (beadsIssue) {
                ...
                if (!createdBeadsIds.has(beadsIssue.id)) {
                  ...
                  if (!existsInArray) {
                    ...
                    // Keep lookup maps current for subsequent iterations
                    beadsLookups.byId.set(beadsIssue.id, beadsIssue);
                    const hulyId = findHulyIdentifier(beadsIssue);
                    if (hulyId) beadsLookups.byHulyId.set(hulyId, beadsIssue);
                    const normTitle = normalizeTitleForComparison(beadsIssue.title);
                    if (normTitle) beadsLookups.byTitle.set(normTitle, beadsIssue);
                  }
                }
              }
            };
            ...
            if (hulyIssues.length > 0) {
              await processBatch(hulyIssues, 1, processPhase3aIssue);
            }

            // Only re-fetch Beads if Phase 3a created new issues (CLI call is expensive)
            const beadsIssuesForPhase3b =
              createdBeadsIds.size > 0 ? await listBeadsIssues(gitRepoPath) : beadsIssues;

            // Phase 3b: Beads → Huly (check for status changes in beads)
            log.info(
              { count: beadsIssuesForPhase3b.length, phase: '3b', project: projectIdentifier },
            ...
            for (const beadsIssue of beadsIssuesForPhase3b) {
            ...
            const processPhase3bIssue = async beadsIssue => {
            ...
            if (beadsIssuesForPhase3b.length > 0) {
              await processBatch(
                beadsIssuesForPhase3b,
                config.sync.maxWorkers,
                processPhase3bIssue
              );
            }
            ...
            phaseCounts.phase3 = {
              beadsCreated: createdBeadsIds.size,
              beadsUpdated: phase3UpdatedIssues.size,
              beadsTotal: beadsIssuesForPhase3b.length,
            };
          }
        } else {
          log.debug(
      ...
      if (config.bookstack?.enabled && bookstackService) {
        const filesystemPath = determineGitRepoPath(hulyProject);
        if (filesystemPath) {
          if (config.bookstack.bidirectionalSync) {
            try {
              const biResult = await bookstackService.syncBidirectional(
                projectIdentifier,
                filesystemPath
              );
              ...
              if (biResult.skipped) {
                log.debug(
                  { project: projectIdentifier, reason: biResult.reason },
                  'BookStack bidirectional sync skipped'
                );
              } else {
                log.info(
                  {
                    project: projectIdentifier,
                    exported: biResult.exported,
                    imported: biResult.imported,
                    conflicts: biResult.conflicts,
                    remoteDeleted: biResult.remoteDeleted,
                  },
                  'BookStack bidirectional sync completed'
                );
              }
            } catch (biError) {
              log.error(
                { err: biError, project: projectIdentifier },
                'Bidirectional sync failed, falling back to independent export/import'
              );

              try {
                await bookstackService.syncExport(projectIdentifier, filesystemPath);
              } catch (exportFallbackError) {
                log.error(
                  { err: exportFallbackError, project: projectIdentifier },
                  'BookStack export fallback also failed'
                );
              }

              if (config.bookstack.importOnSync) {
                try {
                  await bookstackService.syncImport(projectIdentifier, filesystemPath);
                } catch (importFallbackError) {
                  log.error(
                    { err: importFallbackError, project: projectIdentifier },
                    'BookStack import fallback also failed'
                  );
                }
              }
            }
            ...
            try {
              const result = await bookstackService.syncExport(projectIdentifier, filesystemPath);

              if (result.skipped) {
                log.debug(
                  { project: projectIdentifier, reason: result.reason },
                  'BookStack sync skipped'
                );
              } else if (result.success) {
                log.info(
                  { project: projectIdentifier, pages: result.pages?.length || 0 },
              ...
              log.error(
                { err: bookstackError, project: projectIdentifier },
                'BookStack sync failed (Phase 4 isolated)'
              );
            }

            if (config.bookstack.importOnSync) {
              try {
                const importResult = await bookstackService.syncImport(
                  projectIdentifier,
                  filesystemPath
                );
                ...
                if (importResult.skipped) {
                  log.debug(
                    { project: projectIdentifier, reason: importResult.reason },
                    'BookStack import skipped'
                  );
                  ...
                  log.warn(
                    ...
                    'BookStack import had failures'
                  );
                }
              } catch (bookstackImportError) {
                log.error(
                  { err: bookstackImportError, project: projectIdentifier },
                  'BookStack import failed (Phase 4b isolated)'
                );
              }
            }
          }
      ...
      phaseTimings.phase4 = Date.now() - phase4Start;
      ...
      log.info(
        {
          ...
          phases: phaseTimings,
          counts: phaseCounts,
          hulyIssues: hulyIssues.length,
          vibeTasks: vibeTasks.length,
        },
 ...
 */
export function createSyncOrchestrator(db, config, lettaService = null, bookstackService = null) {
  warnLegacyUsage();
  return {
    sync: (hulyClient, vibeClient) =>
      syncHulyToVibe(hulyClient, vibeClient, db, config, lettaService, null, bookstackService),
  };
}
