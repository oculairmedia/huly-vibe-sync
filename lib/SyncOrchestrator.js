            ...
            if (gitRepoPath && process.env.LETTA_UPLOAD_PROJECT_FILES === 'true') {
              try {
                ...
                if (!lettaInfo.letta_folder_id) {
                  ...
                  // Upload key project files (only on first folder creation)
                  const files = await lettaService.discoverProjectFiles(gitRepoPath);
                  if (files.length > 0) {
                    log.info({ project: projectIdentifier, fileCount: files.length }, 'Uploading project files to Letta');
                    await lettaService.uploadProjectFiles(folder.id, gitRepoPath, files);
                  }
                  
                  log.info({ project: projectIdentifier, folderId: folder.id }, 'Letta folder created and attached');
                } else {
        ...
        if (!existingTask) {
          ...
          if (createdTask) {
            db.upsertIssue({
              ...
              huly_modified_at: hulyIssue.modifiedOn || Date.now(),
            });
          }
        } else {
