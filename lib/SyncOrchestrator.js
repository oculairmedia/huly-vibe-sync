...
export async function syncHulyToVibe(
  ...
  try {
    ...
    const processProject = async hulyProject => {
      ...
      // Fetch issues from Huly (with cursor-based incremental sync)
      const { issues: hulyIssues, syncMeta } = await fetchHulyIssues(
        hulyClient,
        projectIdentifier,
        config,
        db,  // Pass db for cursor-based incremental sync
      );
