export function registerBeadsRoutes(app, deps) {
  ...
  // Called periodically by beads-mutation-watcher.mjs with ALL issues
  // from a workspace. Triggers a full BeadsFileChangeWorkflow which
  // diffs against sync DB state and re-syncs any discrepancies.
  ...
  app.registerRoute({
    ...
    handle: async ({ req, res }) => {
      try {
        const body = await parseJsonBody(req);
        const { projectId, issues } = body;

        if (!projectId || !Array.isArray(issues)) {
          sendError(res, 400, 'Missing required fields: projectId, issues (array)');
          return;
        }
        ...
        const beadsIssues = issues.map(i => ({
          id: i.id,
          title: i.title,
          status: i.status,
          priority: i.priority,
        ...
        const { workflowId } = await scheduleBeadsFileChange({
          ...
          gitRepoPath: matched.filesystem_path,
          beadsIssues,
          changedFiles: [`reconciliation:${issues.length}-issues`],
          timestamp: new Date().toISOString(),
        });
