...
function getIssueGraph(issue, projectIssues) {
  ...
  const blocks = projectIssues
    .filter(
      (candidate) =>
        candidate.parent_huly_id === issue.huly_id || candidate.parent_vibe_id === issue.vibe_task_id,
    )
    .map((candidate) => ({
...
function serializeAndroidIssue(issue, projectIssues = [], options = {}) {
  ...
  return {
    ...
    summary: issue.summary || String(issue.description || '').split('\n').find(Boolean) || null,
...
async function handleIssueMutation({ req, res, pathname, parseJsonBody, sendJson, sendError, logger, db, beadsIssueService, action }) {
  if (!db) {
...
function getProjectFromDb(db, identifier) {
  return db.getProject?.(identifier) || null;
}

function getProjectIssuesFromDb(db, identifier) {
  return db.getProjectIssues?.(identifier) || [];
}
...
export function registerProjectRoutes(app, deps) {
  ...
  app.registerRoute({
    ...
    handle: async ({ res, pathname }) => {
      ...
      try {
        ...
        try {
          issues = getProjectIssuesFromDb(db, identifier);
        } catch (error) {
          logger.error({ err: error, project_identifier: identifier }, 'Failed to hydrate tracker');
          trackerOptions = {
  ...
  app.registerRoute({
    ...
    handle: async ({ req, res, pathname }) => {
      ...
      try {
        const identifier = decodeURIComponent(pathname.split('/')[3]);
        const project =
          getProjectFromDb(db, identifier) || projectRegistry?.getProject?.(identifier);
        ...
        sendJson(res, 200, {
          ...
          provisioning: result,
          beads_remote: serializeBeadsRemoteStatus(db?.getProject?.(identifier) || project),
          timestamp: new Date().toISOString(),
        });
      } catch (error) {
    ...
    handle: async ({ res, pathname, url }) => {
      ...
      try {
        ...
        try {
          ...
          logger.error({ err: error, project_identifier: identifier }, 'Failed to hydrate agents');
          lettaInfo = {};
  ...
  app.registerRoute({
    ...
    handle: async ({ res, pathname, url }) => {
      ...
      try {
        ...
        try {
          issues = getProjectIssuesFromDb(db, identifier).filter((issue) => {
          ...
          logger.error(
            { err: error, project_identifier: identifier },
  ...
  app.registerRoute({
    ...
    handle: async ({ res, pathname, url }) => {
      ...
      try {
        const projectIdentifier = decodeURIComponent(pathname.split('/')[3]);
        ...
        const sorted = [...filtered].sort((a, b) => {
          if (sort === 'updated') return toNumber(getIssueTimestamp(b)) - toNumber(getIssueTimestamp(a));
