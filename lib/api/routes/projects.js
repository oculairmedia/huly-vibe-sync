...
function formatZonedIso(date, timezone) {
  ...
  return `${String(parts.year).padStart(4, '0')}-${String(parts.month).padStart(2, '0')}-${String(
    ...
    2,
    '0',
  )}:${String(parts.second).padStart(2, '0')}${offset}`;
}

function getBucketLabel(date, timezone, granularity) {
  if (granularity === 'month') {
    return new Intl.DateTimeFormat('en-US', { timeZone: timezone, month: 'short', year: 'numeric' })
      .format(date);
  }
...
function getStartOfBucket(date, timezone, granularity) {
  const parts = getZonedParts(date, timezone);
  if (granularity === 'month') {
  ...
  if (granularity === 'week') {
    const localNoonUtc = zonedLocalToUtcMs(
      { year: parts.year, month: parts.month, day: parts.day, hour: 12, minute: 0, second: 0 },
      timezone,
    );
    const weekday = new Date(localNoonUtc).getUTCDay() || 7;
    return zonedLocalToUtcMs(
      {
        year: parts.year,
        month: parts.month,
        day: parts.day - (weekday - 1),
        hour: 0,
        minute: 0,
        second: 0,
      },
...
function addBucket(startMs, timezone, granularity) {
  const parts = getZonedParts(new Date(startMs), timezone);
  if (granularity === 'month') {
    return zonedLocalToUtcMs(
      { year: parts.year, month: parts.month + 1, day: 1, hour: 0, minute: 0, second: 0 },
      timezone,
    );
  }
  return zonedLocalToUtcMs(
    {
      year: parts.year,
      month: parts.month,
      day: parts.day + (granularity === 'week' ? 7 : 1),
      hour: 0,
      minute: 0,
      second: 0,
    },
...
function buildAnalyticsBuckets(rangeStartMs, rangeEndMs, timezone, granularity) {
  const buckets = [];
...
function hasBeadsProjectionMetadata(project) {
  return Boolean(
    project &&
      (Object.prototype.hasOwnProperty.call(project, 'beads_remote_status') ||
        Object.prototype.hasOwnProperty.call(project, 'beads_remote_url')),
  );
}

function shouldPreferBeadsIssueSource(project) {
  return hasBeadsProjectionMetadata(project) && hasAccessibleBeadsDatabase(project);
}
...
async function getProjectIssuesWithBeadsFallback({
  ...
  const shouldHydrateFromBeads =
    beadsAdapter &&
    project?.filesystem_path &&
    (dbIssues.length === 0 || shouldPreferBeadsIssueSource(project));

  if (!shouldHydrateFromBeads) {
...
function issueMatchesAnalyticsFilters(issue, filters) {
  ...
  if (filters.assignee && String(issue.assignee || issue.owner || '') !== filters.assignee) {
    return false;
  }
  if (filters.labels.length > 0) {
    const issueLabels = new Set(parseIssueList(issue.labels || issue.tags));
    if (!filters.labels.every((label) => issueLabels.has(label))) return false;
  }
  return true;
}
...
function getIssueCompletedMs(issue) {
  if (normalizeIssueStatus(issue.status) !== 'closed') return null;
  return toDateMs(issue.closed_at || issue.closedAt || issue.completed_at || issue.completedAt);
}

function getTimelinePagination(url) {
...
function getAnalyticsRange(url) {
...
function incrementBucket(buckets, timestampMs, field) {
...
function serializeCompletedTimelineIssue(issue, completedMs) {
  return {
    issueId: issue.identifier || issue.id,
    title: issue.title,
    status: normalizeIssueStatus(issue.status),
    statusLabel: issue.status || normalizeIssueStatus(issue.status),
...
function buildIssueAnalytics({ issues, projectIssues, range, url }) {
  const filters = parseAnalyticsFilters(url);
  const filteredIssues = issues.filter((issue) => issueMatchesAnalyticsFilters(issue, filters));
  const createdBuckets = buildAnalyticsBuckets(
    range.rangeStartMs,
    range.rangeEndMs,
    range.timezone,
    range.granularity,
  ).map((bucket) => ({ ...bucket, createdCount: 0 }));
  const completedBuckets = buildAnalyticsBuckets(
    range.rangeStartMs,
    range.rangeEndMs,
    range.timezone,
    range.granularity,
  ).map((bucket) => ({ ...bucket, completedCount: 0 }));
  ...
  const summary = {
    ...
    blockedCount: 0,
    readyCount: 0,
    totalCreatedInRange: 0,
    totalCompletedInRange: 0,
  };

  for (const issue of filteredIssues) {
    const status = normalizeIssueStatus(issue.status);
    if (status === 'closed') summary.completedCount += 1;
    ...
    if (serializeAndroidIssue(issue, projectIssues).ready) summary.readyCount += 1;

    const createdMs = getIssueCreatedMs(issue);
    if (createdMs !== null && createdMs >= range.rangeStartMs && createdMs < range.rangeEndMs) {
      summary.totalCreatedInRange += 1;
      incrementBucket(createdBuckets, createdMs, 'createdCount');
    }

    const completedMs = getIssueCompletedMs(issue);
    if (
      completedMs !== null &&
      completedMs >= range.rangeStartMs &&
      completedMs < range.rangeEndMs
    ) {
      summary.totalCompletedInRange += 1;
      incrementBucket(completedBuckets, completedMs, 'completedCount');
      completedTimeline.push(serializeCompletedTimelineIssue(issue, completedMs));
    }
  }

  completedTimeline.sort((a, b) => toDateMs(b.completedAt) - toDateMs(a.completedAt));
  ...
  return {
    createdBuckets: createdBuckets.map(({ startMs: _startMs, endMs: _endMs, ...bucket }) => bucket),
    completedBuckets: completedBuckets.map(
      ({ startMs: _startMs, endMs: _endMs, ...bucket }) => bucket,
    ),
    ...
    timelinePage: {
      limit,
      has_more: hasMore,
      total_known: completedTimeline.length,
    },
  };
}
...
export function registerProjectRoutes(app, deps) {
  ...
  app.registerRoute({
    match: ({ pathname, method }) =>
      method === 'GET' && /^\/api\/projects\/[^/]+\/issue-analytics$/.test(pathname),
    handle: async ({ res, pathname, url }) => {
      if (!db) {
        sendError(res, 503, 'Database not available');
        return;
      ...
      try {
        ...
        const projectIssues = hydrated.issues;
        const analytics = buildIssueAnalytics({ issues: projectIssues, projectIssues, range, url });
        ...
        sendJson(res, 200, {
          ...
          isPartial: true,
          etag: getSubresourceEtag(project, 'issue-analytics', project.last_sync_at),
          data_freshness: hydrated.error
            ? getUnavailableFreshness(project.last_sync_at, hydrated.error)
            : getDataFreshness(project.last_sync_at, { source: hydrated.source }),
        ...
        const isValidationError = /rangeEnd|granularity|time zone|timezone/i.test(
          error.message || '',
        );
        sendError(res, isValidationError ? 400 : 500, 'Failed to fetch issue analytics', {
