 ...
 */
export function buildProjectMeta(hulyProject, repoPath, gitUrl) {
  return {
...
export function buildBoardConfig() {
  return {
    ...
    workflow: {
      description: 'Bidirectional sync between Huly and Beads',
 ...
 */
export function buildBoardMetrics(hulyIssues) {
  const statusCounts = {
    Backlog: 0,
    Todo: 0,
    'In Progress': 0,
  ...
  const wip = statusCounts['In Progress'] + statusCounts['In Review'];

  const total = hulyIssues.length;
  ...
  return {
    ...
    active_tasks: statusCounts.Todo + statusCounts['In Progress'] + statusCounts['In Review'],
 ...
 */
export function buildHotspots(hulyIssues) {
  const hotspots = {
    blocked_items: [],
    ageing_wip: [],
    high_priority_todo: [],
 ...
 */
export function buildBacklogSummary(hulyIssues) {
  const todoItems = hulyIssues.filter(issue => {
    const status = issue.status || 'Backlog';
  ...
  const topItems = todoItems.slice(0, 15).map(issue => ({
    id: issue.identifier || issue.id,
