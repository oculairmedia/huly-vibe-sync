 ...
 * - Scratchpad
 *
 * Data source: normalized issue snapshots from sync services.
 * These builders stay transport-agnostic and only consume already-shaped issue data.
 */
 ...
 * Documents the issue workflow and priority system
 *
...
/**
 * Build board metrics snapshot from issue data
 *
...
/**
 * Build recent activity block from issue activity snapshots
...
/**
 * Build components summary block from issue types
...
export function buildComponentsSummary(issues) {
  ...
  const typesList = Array.from(typeStats.values()).sort((a, b) => b.issue_count - a.issue_count);
 ...
 *
 * @param {Array} currentIssues - Current issues from this sync
  ...
  previousIssues.forEach(issue => {
    if (!currentMap.has(issue.identifier)) {
      changes.closed_issues.push({
        identifier: issue.identifier,
        title: issue.title,
        last_status: issue.status || 'unknown',
      });
    }
  });
...
export function buildHotspotsFromSQL({ blocked, agingWip, highPriority }) {
  ...
  const ageingWipItems = (agingWip || []).map(issue => {
    const updatedAt = issue.updated_at ? new Date(issue.updated_at).getTime() : now;
...
export function buildComponentsSummaryFromSQL(typeStats) {
  ...
  const typesList = Array.from(typeMap.values()).sort((a, b) => b.issue_count - a.issue_count);
