...
export class IssueRepository {
  ...
  upsertIssue(issue) {
    ...
    const stmt = this.db.prepare(`
      INSERT INTO issues (
        ...
        parent_huly_id, parent_vibe_id, sub_issue_count,
        content_hash, huly_content_hash
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  getIssueByVibeTaskId(projectIdentifier, vibeTaskId) {
    return this.db
      .prepare(
        'SELECT * FROM issues WHERE project_identifier = ? AND vibe_task_id = ? ORDER BY updated_at DESC LIMIT 1'
      )
  ...
  markDeletedFromVibe(identifier) {
    const stmt = this.db.prepare(`
      UPDATE issues SET deleted_from_vibe = 1, vibe_task_id = NULL, updated_at = ?
      WHERE identifier = ?
    `);
    return stmt.run(Date.now(), identifier);
  }
