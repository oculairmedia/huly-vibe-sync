...
function rebuildIssuesTableWithoutBeadsColumns(db) {
  const columns = db.prepare('PRAGMA table_info(issues)').all();
  const columnNames = columns.map(c => c.name);
  const legacyColumns = [
    'beads_issue_id',
  ...
  if (!legacyColumns.some(column => columnNames.includes(column))) {
    return;
  }

  console.log('[DB] Rebuilding issues table without legacy beads columns');

  db.exec(`
    DROP INDEX IF EXISTS idx_issues_beads_id;
    DROP INDEX IF EXISTS idx_issues_parent_beads;
    DROP INDEX IF EXISTS idx_issues_deleted_from_beads;

    ALTER TABLE issues RENAME TO issues_legacy_beads;
    ...
    SELECT
      ...
      parent_huly_id,
      parent_vibe_id,
      COALESCE(sub_issue_count, 0),
      content_hash,
      huly_content_hash
    FROM issues_legacy_beads;

    DROP TABLE issues_legacy_beads;
...
/**
 * Add ProjectRegistry columns to projects table.
 * @param {import('better-sqlite3').Database} db
 */
export function migrateProjectRegistryColumns(db) {
  const columns = db.prepare('PRAGMA table_info(projects)').all();
  const columnNames = columns.map(c => c.name);

  if (!columnNames.includes('tech_stack')) {
    console.log('[DB] Adding tech_stack column to projects table');
    db.exec(`ALTER TABLE projects ADD COLUMN tech_stack TEXT`);
  }

  if (!columnNames.includes('last_scan_at')) {
    console.log('[DB] Adding last_scan_at column to projects table');
    db.exec(`ALTER TABLE projects ADD COLUMN last_scan_at INTEGER`);
  }

  if (!columnNames.includes('mcp_enabled')) {
    console.log('[DB] Adding mcp_enabled column to projects table');
...
export function runAllMigrations(db) {
  rebuildIssuesTableWithoutBeadsColumns(db);
