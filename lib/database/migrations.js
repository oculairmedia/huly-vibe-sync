...
/**
 * Add Beads/DoltHub remote provisioning metadata to projects.
 * @param {import('better-sqlite3').Database} db
 */
export function migrateProjectBeadsRemoteColumns(db) {
  const columns = db.prepare('PRAGMA table_info(projects)').all();
  const columnNames = columns.map((c) => c.name);

  const addColumn = (name, definition) => {
    if (!columnNames.includes(name)) {
      console.log(`[DB] Adding ${name} column to projects table`);
...
/**
 * Store idempotency keys for offline-safe issue mutations.
 * @param {import('better-sqlite3').Database} db
 */
export function migrateIssueMutationIdempotencyTable(db) {
  db.exec(`
