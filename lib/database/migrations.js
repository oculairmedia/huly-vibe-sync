...
export function migrateProjectRegistryColumns(db) {
  ...
  if (!columnNames.includes('beads_issue_count')) {
    console.log('[DB] Adding beads_issue_count column to projects table');
    db.exec(`ALTER TABLE projects ADD COLUMN beads_issue_count INTEGER DEFAULT 0`);
  }

  if (!columnNames.includes('beads_prefix')) {
    console.log('[DB] Adding beads_prefix column to projects table');
    db.exec(`ALTER TABLE projects ADD COLUMN beads_prefix TEXT`);
  }
