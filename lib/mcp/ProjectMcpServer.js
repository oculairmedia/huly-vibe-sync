 ...
 * @param {import('../ProjectRegistry.js').ProjectRegistry} [params.registry] - ProjectRegistry instance
 * @returns {McpServer} MCP server instance
 */
export function createProjectMcpServer({ db, logger: parentLogger, registry }) {
    ...
    async ({ entity, mode, project_identifier, issue_id, search_term, status_filter }) => {
      try {
        ...
        if (entity === 'project') {
          ...
          } else if (mode === 'search' && registry) {
            const term = (search_term || '').toLowerCase();
            const all = registry.getProjects({});
            result = all.filter(
              p =>
